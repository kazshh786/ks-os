...
async function registerSecurity(fastify: FastifyInstance) {
  ...
  const originAllowed = createCorsOriginAuthorizer({
  ...
  // Invalid browser origins are denied without turning the request into a 500.
  await fastify.register(cors, {
    origin: (origin, cb) => {
      void originAllowed(origin)
        .then(allowed => cb(null, allowed))
        .catch(error => {
          fastify.log.error({ err: error, origin }, 'CORS origin verification failed');
          cb(null, false);
        });
    },
