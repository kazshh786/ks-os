...
// utcMonthPeriod is exported by contracts, but database re-exports the contracts
// in the application build. Keep the local shape explicit for transaction safety.
...
type SourcePage = typeof sitePages.$inferSelect;
type SourceSection = typeof siteSections.$inferSelect;
    ...
    return this.database.transaction(async tx => {
      ...
      for (const section of sourceSections) {
        ...
        const explicitServiceGrid = ['SERVICE_GRID', 'FEATURED_SERVICES'].includes(section.sectionType)
          && Array.isArray(content.serviceReferences);
        if (explicitServiceGrid && !content.serviceReferences.includes(serviceReference)) {
          content.serviceReferences = [...content.serviceReferences, serviceReference];
        }
