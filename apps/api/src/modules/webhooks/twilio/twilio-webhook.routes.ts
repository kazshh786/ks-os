...
import { clients, getDatabase, reviewInvitations, smsOutbox, tenants, twilioWebhookEvents } from '@ks-os/database';
...
export const twilioWebhookRoutes: FastifyPluginAsync = async fastify => {
  fastify.post('/status', async (request, reply) => {
    ...
    const db=getDatabase(); const [current]=await db.select().from(smsOutbox).where(eq(smsOutbox.providerMessageSid,sid)).limit(1); if(!current) return reply.code(204).send();
  ...
  fastify.post('/inbound', async (request, reply) => {
    ...
    if(!inserted.length) return reply.type('text/xml').send(xml('Your message was already processed.'));
    const keyword=(body.OptOutType || body.Body || '').trim().toUpperCase(); const phone=body.From;
    ...
    return reply.type('text/xml').send(xml(help.has(keyword) ? `KS OS sends automated appointment and form messages.${contact}` : `KS OS automated messages cannot process replies.${contact}`));
  });
};
