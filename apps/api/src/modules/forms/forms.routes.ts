...
import { CreateFormAssignmentSchema, FormAnalyticsEventSchema, FormAssignmentIdParamsSchema, FormAssignmentListQuerySchema, FormDraftInputSchema, FormIdParamsSchema, FormSubmissionIdParamsSchema, FormSubmissionListQuerySchema, FormVersionParamsSchema, PublicFormSubmissionSchema, PublicFormTokenParamsSchema, ReviewFormSubmissionSchema, SaveFormDraftSchema } from '@ks-os/contracts';
...
export async function formSubmissionRoutes(app:FastifyInstance){const service=new FormsService();app.get('/',async(request)=>({data:await service.listSubmissions(actor(request),FormSubmissionListQuerySchema.parse(request.query))}));app.get('/:submissionId',async(request)=>{const{submissionId}=FormSubmissionIdParamsSchema.parse(request.params);return{data:await service.getSubmission(actor(request),submissionId)};});app.patch('/:submissionId/review',async request=>{const{submissionId}=FormSubmissionIdParamsSchema.parse(request.params);return{data:await service.reviewSubmission(actor(request),submissionId,ReviewFormSubmissionSchema.parse(request.body))};});}

export async function publicFormRoutes(app:FastifyInstance){const service=new FormsService();app.get('/:token',{config:{rateLimit:{max:30,timeWindow:'1 minute'}}},async(request)=>{const{token}=PublicFormTokenParamsSchema.parse(request.params);return{data:await service.getPublic(token)};});app.put('/:token/draft',{bodyLimit:262144,config:{rateLimit:{max:30,timeWindow:'1 minute'}}},async request=>{const{token}=PublicFormTokenParamsSchema.parse(request.params);return{data:await service.saveDraft(token,SaveFormDraftSchema.parse(request.body))};});app.get('/resume/:token',{config:{rateLimit:{max:20,timeWindow:'1 minute'}}},async request=>{const{token}=PublicFormTokenParamsSchema.parse(request.params);return{data:await service.resumeDraft(token)};});app.post('/:token/analytics',{bodyLimit:8192,config:{rateLimit:{max:60,timeWindow:'1 minute'}}},async(request,reply)=>{const{token}=PublicFormTokenParamsSchema.parse(request.params);await service.recordAnalytics(token,FormAnalyticsEventSchema.parse(request.body));return reply.code(204).send();});app.post('/:token/submissions',{bodyLimit:262144,config:{rateLimit:{max:10,timeWindow:'1 minute'}}},async(request,reply)=>{const{token}=PublicFormTokenParamsSchema.parse(request.params);return reply.code(201).send({data:await service.submitPublic(token,PublicFormSubmissionSchema.parse(request.body))});});}
