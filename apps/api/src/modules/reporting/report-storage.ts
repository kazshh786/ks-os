...
export interface ReportStorageLike { upload(path:string,body:Buffer):Promise<void>; signedUrl(path:string,expiresInSeconds:number,filename:string):Promise<string>; remove(paths:string[]):Promise<void>; }
...
export class SupabaseReportStorage implements ReportStorageLike {
  async upload(path:string,body:Buffer){const{error}=await getAdmin().storage.from(REPORT_EXPORT_BUCKET).upload(path,body,{contentType:'text/csv',upsert:false,cacheControl:'private, max-age=0'});if(error)throw Object.assign(new Error('Private export upload failed.'),{code:'EXPORT_UPLOAD_FAILED',cause:error});}
