...
export const Login: React.FC = () => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || body.data?.next === 'NO_ACCESS') {
        ...
        navigate('/access-denied', { replace: true }); return;
      }
  ...
  return <main className="flex min-h-screen items-center justify-center bg-slate-950 p-6 text-white">
    <section className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
      ...
      <div className="mt-5 flex items-center justify-between text-xs font-bold"><Link to="/forgot-password" className="text-indigo-300">Forgot password?</Link><Link to="/agency/login" className="text-slate-400">Agency sign in</Link></div>
