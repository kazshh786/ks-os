...
export function BookingPageSettings() {
  ...
  const publish = async () => {
    ...
    setError('');
    try {
      const saved = await getDataProvider().setBookingPagePublished(!page.published);
      setPage(saved);
  ...
  return <main className="space-y-5">
    <header className="flex flex-col justify-between gap-4 rounded-3xl bg-slate-950 p-6 text-white lg:flex-row lg:items-end">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-black">Universal booking experience</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Every business uses the same platform-controlled booking layout, components and conversion flow. Only the logo and colour scheme change.</p>
      </div>
    ...
    <div className="grid gap-5 2xl:grid-cols-[420px_minmax(0,1fr)]">
      <div className="space-y-5">
        <section className="rounded-2xl border bg-white p-5">
          <div className="flex items-start gap-3"><Palette className="mt-0.5 h-5 w-5 text-indigo-600" /><div><h2 className="text-lg font-black">Brand controls</h2><p className="mt-1 text-xs leading-5 text-slate-500">These are the only visual settings. Typography, spacing, steps, cards, summary and payment handoff stay identical across every business.</p></div></div>
          <div className="mt-5 grid gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="text-sm font-bold">Primary colour<input type="color" value={page.theme.primaryColor} onChange={event => update('theme', { ...page.theme, primaryColor: event.target.value })} className="mt-1 h-12 w-full rounded-lg border p-1" /></label>
              <label className="text-sm font-bold">Accent colour<input type="color" value={page.theme.accentColor} onChange={event => update('theme', { ...page.theme, accentColor: event.target.value })} className="mt-1 h-12 w-full rounded-lg border p-1" /></label>
            </div>
          ...
          <div className="mt-5 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-xs leading-5 text-indigo-950"><div className="flex gap-2"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><p><strong>Platform standard:</strong> business name, live services, staff, locations and prices come from verified workspace data. Visual page copy and layout are not tenant-editable.</p></div></div>
        </section>

        <section className="rounded-2xl border bg-white p-5">
          <div className="flex items-start gap-3"><LockKeyhole className="mt-0.5 h-5 w-5 text-indigo-600" /><div><h2 className="text-lg font-black">Booking and payment rules</h2><p className="mt-1 text-xs leading-5 text-slate-500">Operational rules can change without changing the universal customer experience.</p></div></div>
          <div className="mt-5 grid grid-cols-2 gap-4">
            ...
            <label className="text-sm font-bold">Future window (days)<input type="number" min={1} max={730} value={page.bookingRules.maximumFutureDays} onChange={event => update('bookingRules', { ...page.bookingRules, maximumFutureDays: Number(event.target.value) })} className="mt-1 w-full rounded-xl border p-3" /></label>
          ...
          <div className="mt-4 flex items-start gap-2 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /><p>When payment is due, the booking flow hands the customer to Stripe Checkout. Card details never pass through KS OS forms.</p></div>
        </section>
      ...
      <section className="min-w-0 rounded-2xl border bg-slate-100 p-3">
        <header className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div><p className="text-xs font-black uppercase tracking-wide text-slate-500">Universal live preview</p><p className="text-sm text-slate-600">Interactions are isolated from booking creation.</p></div>
          <div className="flex rounded-lg border bg-white p-1">
            <button onClick={() => setPreviewWidth('desktop')} aria-label="Desktop preview" aria-pressed={previewWidth === 'desktop'} className={`rounded-md p-2 ${previewWidth === 'desktop' ? 'bg-slate-900 text-white' : ''}`}><Monitor className="h-4 w-4" /></button>
            <button onClick={() => setPreviewWidth('tablet')} aria-label="Tablet preview" aria-pressed={previewWidth === 'tablet'} className={`rounded-md p-2 ${previewWidth === 'tablet' ? 'bg-slate-900 text-white' : ''}`}><Tablet className="h-4 w-4" /></button>
            <button onClick={() => setPreviewWidth('mobile')} aria-label="Mobile preview" aria-pressed={previewWidth === 'mobile'} className={`rounded-md p-2 ${previewWidth === 'mobile' ? 'bg-slate-900 text-white' : ''}`}><Smartphone className="h-4 w-4" /></button>
          </div>
        </header>
        <div className="mx-auto overflow-auto rounded-xl bg-slate-50 p-2 transition-all" style={{ maxWidth: previewWidths[previewWidth] }}>
          <PublicBookingFlow slug={loadedSlug} preview pageOverride={pageOverride} />
        </div>
      </section>
    </div>
  </main>;
