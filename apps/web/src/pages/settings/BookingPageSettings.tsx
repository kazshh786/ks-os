...
export function BookingPageSettings() {
  ...
  return <main className="space-y-5">
    <header className="flex flex-col justify-between gap-4 rounded-3xl bg-slate-950 p-4 text-white sm:p-6 lg:flex-row lg:items-end"><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-300">Online booking</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">Public booking page</h1><p className="mt-2 max-w-2xl text-sm text-slate-300">Every business has a platform URL automatically. Preview uses the real customer components and never creates bookings.</p></div><div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><button onClick={() => navigator.clipboard.writeText(page.publicUrl).then(() => setMessage('Booking link copied.'))} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white/10 px-4 py-2 text-sm font-bold"><Clipboard className="h-4 w-4" />Copy link</button><a href={page.publicUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white/10 px-4 py-2 text-sm font-bold"><ExternalLink className="h-4 w-4" />Open page</a><button onClick={() => void publish()} disabled={saving} className="min-h-11 rounded-xl bg-indigo-500 px-4 py-2 text-sm font-black">{page.published ? 'Unpublish' : 'Publish'}</button><button onClick={() => void save()} disabled={saving} className="min-h-11 rounded-xl bg-white px-4 py-2 text-sm font-black text-slate-950">{saving ? 'Saving…' : 'Save changes'}</button></div></header>
    ...
    <div className="grid gap-5 2xl:grid-cols-[440px_minmax(0,1fr)]">
      <div className="space-y-5">
        ...
        <section className="rounded-2xl border bg-white p-5"><h2 className="text-lg font-black">Brand and layout</h2><div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Primary colour<input type="color" value={page.theme.primaryColor} onChange={event => update('theme', { ...page.theme, primaryColor: event.target.value })} className="mt-1 h-12 w-full rounded-lg border p-1" /></label><label className="text-sm font-bold">Accent colour<input type="color" value={page.theme.accentColor} onChange={event => update('theme', { ...page.theme, accentColor: event.target.value })} className="mt-1 h-12 w-full rounded-lg border p-1" /></label><label className="text-sm font-bold">Layout<select value={page.layout} onChange={event => update('layout', event.target.value as BookingPageResponse['layout'])} className="mt-1 min-h-11 w-full rounded-xl border bg-white p-3"><option value="STANDARD">Standard</option><option value="COMPACT">Compact</option><option value="EDITORIAL">Editorial</option></select></label><label className="text-sm font-bold">Corners<select value={page.theme.borderRadius} onChange={event => update('theme', { ...page.theme, borderRadius: event.target.value as BookingPageResponse['theme']['borderRadius'] })} className="mt-1 min-h-11 w-full rounded-xl border bg-white p-3"><option value="compact">Compact</option><option value="medium">Medium</option><option value="rounded">Rounded</option></select></label></div></section>
        <section className="rounded-2xl border bg-white p-5"><h2 className="text-lg font-black">Booking rules</h2><div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Minimum notice (minutes)<input type="number" min={0} value={page.bookingRules.minimumNoticeMinutes} onChange={event => update('bookingRules', { ...page.bookingRules, minimumNoticeMinutes: Number(event.target.value) })} className="mt-1 min-h-11 w-full rounded-xl border p-3" /></label><label className="text-sm font-bold">Future window (days)<input type="number" min={1} max={730} value={page.bookingRules.maximumFutureDays} onChange={event => update('bookingRules', { ...page.bookingRules, maximumFutureDays: Number(event.target.value) })} className="mt-1 min-h-11 w-full rounded-xl border p-3" /></label><label className="text-sm font-bold">Payment requirement<select value={page.paymentSettings.mode} onChange={event => update('paymentSettings', { ...page.paymentSettings, mode: event.target.value as BookingPageResponse['paymentSettings']['mode'] })} className="mt-1 min-h-11 w-full rounded-xl border bg-white p-3"><option value="NONE">No payment</option><option value="PAY_LATER">Pay later</option><option value="DEPOSIT">Deposit</option><option value="FULL">Full payment</option><option value="CUSTOMER_CHOICE">Customer choice</option></select></label><label className="text-sm font-bold">Deposit %<input type="number" min={0} max={100} value={page.paymentSettings.depositPercentage} onChange={event => update('paymentSettings', { ...page.paymentSettings, depositPercentage: Number(event.target.value) })} className="mt-1 min-h-11 w-full rounded-xl border p-3" /></label></div><label className="mt-4 block text-sm font-bold">Cancellation policy<textarea value={page.cancellationSettings.policyText} onChange={event => update('cancellationSettings', { ...page.cancellationSettings, policyText: event.target.value })} rows={3} className="mt-1 w-full rounded-xl border p-3" /></label></section>
