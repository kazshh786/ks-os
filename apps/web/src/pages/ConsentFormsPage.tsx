...
export default function ConsentFormsPage() {
  ...
  useEffect(() => {
    ...
    getDataProvider()
      ...
      .catch(() => {
        setError('Consent forms could not be loaded. No mock data has been substituted.');
      });
    ...
    <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="relative overflow-hidden rounded-[32px] bg-slate-950 p-6 text-white shadow-xl sm:p-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">Beautiful forms that clients can complete anywhere</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Build consent, consultation and intake forms directly in the customer view, publish them to a simple link, and keep every response connected to the client record.</p>
            </div>
            {role === 'owner' && (
          ...
          <div className="relative mt-7 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-1 text-xs font-bold text-slate-400">Total forms</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-2xl font-black text-emerald-300">{publishedCount}</p>
              <p className="mt-1 text-xs font-bold text-slate-400">Live forms</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-2xl font-black text-indigo-200">{submissionCount}</p>
              <p className="mt-1 text-xs font-bold text-slate-400">Secure submissions</p>
            </div>
          </div>
        </section>
      )}
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
        ...
        <div>
          ...
          <p className="mt-1 text-emerald-900/80">Public links open only published versions. Assigned completion links should still be shared only with the intended client.</p>
        </div>
      </div>
      ...
      <section aria-labelledby="form-library-heading">
        <div className="mb-4 flex items-end justify-between gap-4">
          <div>
            <h2 id="form-library-heading" className="text-xl font-black text-slate-950">{assignmentClientId ? 'Published forms' : 'Your form library'}</h2>
            <p className="mt-1 text-sm text-slate-500">{assignmentClientId ? 'Select the form that should be linked to this customer.' : 'Open form details to review completion activity, success-page settings and published versions.'}</p>
          </div>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          {!error && forms.length === 0 && (
            <div className="col-span-full rounded-3xl border-2 border-dashed border-slate-200 bg-white p-12 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Drag fields directly onto the customer view, save the draft and publish it to a simple public link.</p>
              {role === 'owner' && (
