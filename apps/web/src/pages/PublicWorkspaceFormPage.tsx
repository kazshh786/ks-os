      ...
      <section className="overflow-hidden rounded-[32px] border border-slate-200/80 shadow-2xl shadow-slate-300/40" style={{ background: schema.theme.cardColor }}>
        {!review ? <>
          ...
          <div className="px-6 py-7 md:px-9 md:py-9"><dl className="space-y-3">{schema.fields.filter(field => state.get(field.key || field.id)?.visible && !['INFORMATION', 'HEADING', 'DIVIDER', 'HIDDEN'].includes(field.type)).map(field => { const value = answers[field.key || field.id]; return <div key={field.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4"><dt className="text-xs font-black uppercase tracking-wide text-slate-500">{field.label}</dt><dd className="mt-1 font-semibold text-slate-900">{Array.isArray(value) ? value.join(', ') : value === true ? 'Yes' : value === false ? 'No' : String(value ?? 'Not answered')}</dd></div>; })}</dl>
            <section className="mt-6 rounded-2xl border p-5" style={{ borderColor: `${primary}35`, background: `${primary}0d` }}><h2 className="font-black" style={{ color: primary }}>Consent acknowledgement</h2><p className="mt-2 text-sm leading-6 text-slate-700">{data.form.acknowledgementText}</p><label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border bg-white p-4" style={{ borderColor: `${accent}45` }}><input type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)} className="mt-0.5 h-5 w-5 rounded border-slate-300" style={{ accentColor: primary }} /><span className="text-sm font-bold text-slate-800">I have read and accept this acknowledgement.</span></label><label className="mt-4 block text-sm font-black text-slate-800">Full legal name<input value={name} onChange={event => setName(event.target.value)} placeholder="Type your full name" className="mt-2 w-full rounded-2xl border border-slate-200 bg-white p-3.5 font-normal outline-none focus:ring-4" /></label></section>
            {submitError && <p role="alert" className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{submitError}</p>}
          </div>
          <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/70 px-6 py-5 md:px-9"><button type="button" onClick={() => setReview(false)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700"><ArrowLeft className="h-4 w-4" />Edit answers</button><button type="button" onClick={() => void submit()} disabled={!accepted || !name.trim() || submitting} className="rounded-xl px-6 py-3 text-sm font-black text-white shadow-md disabled:opacity-40" style={{ background: brandGradient }}>{submitting ? 'Submitting securely…' : 'Submit consent form'}</button></div>
        </>}
      </section>
