...
export const StaffWorkspaceLayout: React.FC = () => {
  ...
  const primaryAction = canCreateBooking ? <Link to="/app/calendar?create=1" className={`flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 font-black text-white shadow-sm hover:bg-indigo-700 ${collapsed ? 'px-0' : 'gap-2 px-4 text-sm'}`} title={collapsed ? 'Create booking' : undefined}><Plus aria-hidden="true" className="h-5 w-5" />{!collapsed && 'Create booking'}</Link> : undefined;
  ...
  const sidebar = (isMobile = false) => <AppSidebar
    ...
    primaryAction={isMobile && canCreateBooking ? <Link to="/app/calendar?create=1" onClick={() => setMobileOpen(false)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-black text-white"><Plus aria-hidden="true" className="h-5 w-5" />Create booking</Link> : primaryAction}
