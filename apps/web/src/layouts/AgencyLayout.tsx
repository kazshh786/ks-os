...
export const AgencyLayout: React.FC = () => {
  ...
  const sidebar = (isMobile = false) => <AppSidebar
    ...
    secondaryActions={!tenantId ? <div className="grid grid-cols-2 gap-2"><Link to="/agency/tenants/new" className="flex min-h-11 items-center justify-center gap-1 rounded-lg border border-slate-800 p-2 text-[10px] font-bold text-slate-400 hover:text-white"><Plus aria-hidden="true" className="h-3 w-3" />Business</Link>{capabilities.includes('agency.users.manage') && <Link to="/agency/users/new" className="flex min-h-11 items-center justify-center gap-1 rounded-lg border border-slate-800 p-2 text-[10px] font-bold text-slate-400 hover:text-white"><Plus aria-hidden="true" className="h-3 w-3" />Team member</Link>}</div> : <Link to="/agency/tenants" className="flex min-h-11 items-center justify-center gap-2 rounded-lg border border-slate-800 p-2 text-xs font-bold text-slate-400 hover:text-white"><DoorOpen aria-hidden="true" className="h-4 w-4" />Exit business management</Link>}
  ...
  return <div className="flex h-dvh min-h-0 overflow-hidden bg-slate-950 font-sans text-white antialiased">
    ...
    <div className="flex min-w-0 flex-1 flex-col">
      ...
      <main id="main-content" className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain bg-slate-950 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] text-slate-100 sm:p-6 lg:p-8"><Outlet /></main>
    </div>
