    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <a href="/login" className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-300 transition hover:bg-white/5 hover:text-white">Client sign in</a>
          <a href={BOOKING_URL} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950 shadow-[0_10px_35px_rgba(103,232,249,0.2)] transition hover:-translate-y-0.5 hover:bg-cyan-200">Book a consultation <ArrowRight className="h-4 w-4" /></a>
        </div>
  ...
  return (
    <footer className="border-t border-white/10 bg-slate-950 text-slate-300">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 md:grid-cols-[1.4fr_1fr_1fr] lg:px-8">
        <div className="max-w-md"><Logo /><p className="mt-5 text-sm leading-7 text-slate-400">A done-for-you business operating system that brings your website, bookings, customers, communications and operations together—without taking control away from you.</p></div>
        <div><p className="text-xs font-black uppercase tracking-[0.2em] text-white">Explore</p><div className="mt-4 grid gap-3 text-sm"><Link to="/services" className="hover:text-white">All services</Link><a href="/#comparison" className="hover:text-white">Compare costs</a><a href={BOOKING_URL} className="hover:text-white">Book a consultation</a></div></div>
...
function SectionHeading({ eyebrow, title, copy, dark = false, centre = false }: { eyebrow: string; title: string; copy?: string; dark?: boolean; centre?: boolean }) {
  return (
    <div className={`${centre ? 'mx-auto text-center' : ''} max-w-3xl`}>
      <p className={`text-xs font-black uppercase tracking-[0.24em] ${dark ? 'text-cyan-200' : 'text-cyan-700'}`}>{eyebrow}</p>
...
function ServiceCard({ service, featured = false }: { service: Service; featured?: boolean }) {
  ...
  return (
    <Link to={`/services/${service.slug}`} className={`group rounded-[1.75rem] border border-slate-200 bg-white p-7 shadow-[0_18px_60px_rgba(15,23,42,0.06)] transition hover:-translate-y-1 hover:border-cyan-300 hover:shadow-[0_24px_70px_rgba(15,23,42,0.1)] ${featured ? 'lg:col-span-2' : ''}`}>
      <div className="flex items-start justify-between gap-4"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-950 text-cyan-200"><Icon className="h-5 w-5" /></span><ArrowRight className="h-5 w-5 text-slate-300 transition group-hover:translate-x-1 group-hover:text-cyan-700" /></div>
      <p className="mt-7 text-xs font-black uppercase tracking-[0.2em] text-cyan-700">{service.eyebrow}</p>
...
function HomePage() {
  ...
  const metrics: Array<[LucideIcon, string, string, string]> = [[CalendarCheck2, 'Bookings', '42 this week', '+18%'], [UsersRound, 'Active customers', '1,284', '+64'], [Gauge, 'Lead response', '4m 12s', '-31%'], [BarChart3, 'Conversion', '28.6%', '+6.4%']];
  return (
    <>
      <section className="relative overflow-hidden bg-slate-950 text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 py-20 sm:py-28 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-32">
          <div>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href={BOOKING_URL} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-cyan-300 px-6 py-4 text-sm font-black text-slate-950 shadow-[0_15px_50px_rgba(103,232,249,0.22)] transition hover:-translate-y-0.5 hover:bg-cyan-200">Book your systems consultation <ArrowRight className="h-4 w-4" /></a><a href="#comparison" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-6 py-4 text-sm font-black text-white transition hover:bg-white/10">See the cost of separate tools</a></div>
            <div className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm font-bold text-slate-400">{['Built around your workflow', 'One connected customer record', 'Ongoing support included'].map(item => <span key={item} className="inline-flex items-center gap-2"><CircleCheckBig className="h-4 w-4 text-cyan-300" />{item}</span>)}</div>
          </div>
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.07] p-4 shadow-2xl backdrop-blur-xl sm:p-6">
              <div className="flex items-center justify-between border-b border-white/10 pb-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-200">KS OS command centre</p><p className="mt-1 text-sm text-slate-400">One connected view of the business</p></div><div className="flex gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-rose-400" /><span className="h-2.5 w-2.5 rounded-full bg-amber-300" /><span className="h-2.5 w-2.5 rounded-full bg-emerald-300" /></div></div>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">{metrics.map(([Icon, label, value, delta]) => <div key={label} className="rounded-2xl border border-white/10 bg-slate-950/50 p-4"><div className="flex items-center justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-300/10 text-cyan-200"><Icon className="h-4 w-4" /></span><span className="text-xs font-black text-emerald-300">{delta}</span></div><p className="mt-5 text-xs font-bold text-slate-400">{label}</p><p className="mt-1 text-2xl font-black tracking-tight text-white">{value}</p></div>)}</div>
              <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/50 p-4"><div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-400">Connected customer journey</p><span className="text-[10px] font-black uppercase tracking-wider text-cyan-200">Live</span></div><div className="mt-4 flex items-center gap-2 overflow-hidden text-[10px] font-black uppercase tracking-wider text-slate-300">{['Visit', 'Enquiry', 'Booked', 'Reminder', 'Paid', 'Review'].map((step, index) => <div key={step} className="contents"><span className="whitespace-nowrap rounded-lg bg-white/5 px-2.5 py-2">{step}</span>{index < 5 && <ArrowRight className="h-3 w-3 shrink-0 text-cyan-300" />}</div>)}</div></div>
            </div>
          </div>
        </div>
      </section>

      <section className="border-b border-slate-200 bg-white"><div className="mx-auto grid max-w-7xl gap-6 px-5 py-8 text-center sm:grid-cols-3 lg:px-8">{[[Layers3, 'One platform', 'Instead of a patchwork of subscriptions'], [Headphones, 'One accountable partner', 'Instead of five different support teams'], [ShieldCheck, 'Your rules and data', 'Instead of losing control to complexity']].map(([Icon, title, copy]) => { const ItemIcon = Icon as LucideIcon; return <div key={title as string} className="flex items-center justify-center gap-4 sm:text-left"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-950"><ItemIcon className="h-5 w-5" /></span><div><p className="font-black text-slate-950">{title as string}</p><p className="mt-1 text-xs leading-5 text-slate-500">{copy as string}</p></div></div>; })}</div></section>

      <section className="bg-slate-50 px-5 py-20 sm:py-28 lg:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="The complete system" title="Everything your business needs to attract, convert and serve customers" copy="Choose the capabilities you need now. KS OS connects them into one operating system and manages the technical detail behind the scenes." centre /><div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{services.map((service, index) => <ServiceCard key={service.slug} service={service} featured={index === 0} />)}</div></div></section>

      <section id="control" className="overflow-hidden bg-white px-5 py-20 sm:py-28 lg:px-8"><div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2"><div><SectionHeading eyebrow="Managed, not mysterious" title="We run the system. You run the business." copy="Done-for-you should not mean locked out. KS OS separates technical administration from business control, so you get expert implementation without giving up visibility or ownership." /><div className="mt-9 grid gap-5">{[['We design and configure', 'We map the journey, build the workflows, connect the tools and test the complete experience.', WandSparkles], ['You approve and control', 'You decide services, pricing, availability, messaging, permissions and what should—or should not—be automated.', MousePointerClick], ['We monitor and improve', 'We keep the system healthy, respond to issues and help prioritise improvements as your business evolves.', Gauge]].map(([title, copy, Icon]) => { const StepIcon = Icon as LucideIcon; return <div key={title as string} className="flex gap-4 rounded-2xl border border-slate-200 p-5"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-cyan-50 text-cyan-800"><StepIcon className="h-5 w-5" /></span><div><h3 className="font-black text-slate-950">{title as string}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{copy as string}</p></div></div>; })}</div></div><div className="relative rounded-[2rem] bg-slate-950 p-6 text-white shadow-2xl sm:p-9"><div className="absolute right-0 top-0 h-40 w-40 rounded-full bg-cyan-300/10 blur-3xl" /><p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-200">Your control layer</p><h3 className="mt-4 text-3xl font-black tracking-[-0.04em]">Clear decisions. Clear ownership. No black box.</h3><div className="mt-8 grid gap-3 sm:grid-cols-2">{['Your domain and brand', 'Your customer data', 'Your service rules', 'Your team permissions', 'Your communication approvals', 'Your performance dashboards', 'Your payment settings', 'Your change priorities'].map(item => <div key={item} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 text-sm font-bold text-slate-200"><Check className="h-4 w-4 shrink-0 text-cyan-300" />{item}</div>)}</div></div></div></section>

      <section id="comparison" className="bg-slate-950 px-5 py-20 text-white sm:py-28 lg:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="The real cost of disconnected software" title="Stop paying for a stack you still have to operate yourself" copy="Separate subscriptions can look affordable one by one. The cost—and complexity—appears when you need them connected, configured, maintained and supported." dark /><div className="mt-12 overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.04] shadow-2xl"><div className="overflow-x-auto"><table className="w-full min-w-[760px] border-collapse text-left"><thead><tr className="border-b border-white/10 bg-white/[0.04]"><th className="px-6 py-5 text-xs font-black uppercase tracking-[0.18em] text-slate-400">Capability</th><th className="px-6 py-5 text-xs font-black uppercase tracking-[0.18em] text-rose-200">Buying separately</th><th className="px-6 py-5 text-xs font-black uppercase tracking-[0.18em] text-cyan-200">With KS OS</th></tr></thead><tbody>{stackComparison.map(([capability, separate, included]) => <tr key={capability} className="border-b border-white/10 last:border-0"><td className="px-6 py-5 font-bold text-white">{capability}</td><td className="px-6 py-5 text-sm text-slate-300">{separate}</td><td className="px-6 py-5"><span className="inline-flex items-center gap-2 text-sm font-bold text-cyan-100"><CircleCheckBig className="h-4 w-4 text-cyan-300" />{included}</span></td></tr>)}</tbody><tfoot><tr className="bg-cyan-300 text-slate-950"><td className="px-6 py-6 text-lg font-black">Typical combined position</td><td className="px-6 py-6"><span className="block text-2xl font-black">£315–£2,000+/mo</span><span className="text-xs font-bold opacity-70">before implementation projects and internal admin time</span></td><td className="px-6 py-6"><span className="block text-2xl font-black">One tailored plan</span><span className="text-xs font-bold opacity-70">platform, implementation and ongoing support together</span></td></tr></tfoot></table></div></div><p className="mt-4 text-xs leading-6 text-slate-500">Illustrative monthly market ranges based on publicly advertised software categories. Actual costs vary by users, contacts, usage and feature tier. Email, SMS and payment processing usage may carry separate transparent charges.</p></div></section>

      <section className="bg-white px-5 py-20 sm:py-28 lg:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="A structured path to launch" title="From scattered tools to one working operating system" centre /><div className="mt-14 grid gap-5 md:grid-cols-4">{[['01', 'Discover', 'We review your customer journey, tools, bottlenecks and priorities.'], ['02', 'Design', 'We define the system, data, journeys, controls and launch sequence.'], ['03', 'Build', 'We configure, integrate, test and prepare your team for launch.'], ['04', 'Improve', 'We monitor performance and evolve the system with the business.']].map(([number, title, copy]) => <div key={number} className="relative rounded-2xl border border-slate-200 p-6"><span className="text-4xl font-black tracking-[-0.06em] text-slate-200">{number}</span><h3 className="mt-8 text-xl font-black text-slate-950">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{copy}</p></div>)}</div></div></section>

      <section id="faq" className="bg-slate-50 px-5 py-20 sm:py-28 lg:px-8"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.75fr_1.25fr]"><SectionHeading eyebrow="Common questions" title="What working with KS OS actually means" copy="A managed platform should make the business simpler, not create a new dependency you cannot understand." /><div className="grid gap-3">{faqs.map(([question, answer], index) => <div key={question} className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><button type="button" onClick={() => setFaq(faq === index ? null : index)} className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left font-black text-slate-950" aria-expanded={faq === index}><span>{question}</span><ChevronDown className={`h-5 w-5 shrink-0 transition ${faq === index ? 'rotate-180 text-cyan-700' : 'text-slate-400'}`} /></button>{faq === index && <p className="px-6 pb-6 text-sm leading-7 text-slate-600">{answer}</p>}</div>)}</div></div></section>

      <section className="bg-white px-5 py-20 lg:px-8"><div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-gradient-to-br from-cyan-300 via-sky-300 to-indigo-300 p-8 text-slate-950 shadow-[0_28px_90px_rgba(14,165,233,0.2)] sm:p-12 lg:flex lg:items-center lg:justify-between lg:gap-12"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[0.22em]">Start with clarity</p><h2 className="mt-4 text-3xl font-black tracking-[-0.05em] sm:text-5xl">See what your business could run like when everything is connected.</h2><p className="mt-5 max-w-2xl text-base font-semibold leading-7 text-slate-800">Book a systems consultation through the KS Agency booking system. We will identify the highest-impact improvements and recommend a practical path forward.</p></div><a href={BOOKING_URL} className="mt-8 inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-4 text-sm font-black text-white shadow-xl transition hover:-translate-y-0.5 lg:mt-0">Book a consultation <ArrowRight className="h-4 w-4" /></a></div></section>
    </>
  );
}
...
function ServicesIndexPage() {
  ...
  return <><section className="bg-slate-950 px-5 py-20 text-white sm:py-28 lg:px-8"><div className="mx-auto max-w-7xl"><p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-200">KS OS services</p><h1 className="mt-5 max-w-5xl text-5xl font-black tracking-[-0.055em] sm:text-6xl">A complete growth and operations stack, delivered as one managed system.</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-slate-300">Each service works on its own. The real advantage comes when they share customer data, trigger the right workflows and give you one connected view of the business.</p></div></section><section className="bg-slate-50 px-5 py-20 lg:px-8"><div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">{services.map(service => <ServiceCard key={service.slug} service={service} />)}</div></section></>;
}
