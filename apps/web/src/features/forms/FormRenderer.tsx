...
export function FormFieldControl({
  ...
  const details = <>{description && <span id={`${key}-details`} className="mt-1.5 block text-sm font-normal leading-5 text-slate-500">{description}</span>}{error && <span role="alert" className="mt-1.5 block text-sm font-semibold text-red-700">{error}</span>}</>;
  ...
  if (['LONG_TEXT', 'ADDRESS'].includes(field.type)) {
    ...
    control = <div className="mt-3 grid grid-cols-2 gap-2">{['Yes', 'No'].map(option => <button key={option} type="button" disabled={disabled} aria-pressed={isSelected(value, option)} onClick={() => onChange(option)} className={`rounded-2xl border px-4 py-3 text-sm font-black transition ${isSelected(value, option) ? 'border-indigo-600 bg-indigo-600 text-white shadow-md' : 'border-slate-200 bg-white text-slate-700 hover:border-indigo-300 hover:bg-indigo-50'}`}>{option}</button>)}</div>;
    ...
    control = <div className="mt-3 flex flex-wrap gap-2" aria-label={label}>{[1, 2, 3, 4, 5].map(score => <button key={score} type="button" disabled={disabled} aria-label={`${score} out of 5`} aria-pressed={rating === score} onClick={() => onChange(score)} className={`h-11 w-11 rounded-xl border text-xl transition ${rating >= score ? 'border-amber-300 bg-amber-50 text-amber-500' : 'border-slate-200 bg-white text-slate-300 hover:border-amber-200'}`}>★</button>)}</div>;
  } else {
