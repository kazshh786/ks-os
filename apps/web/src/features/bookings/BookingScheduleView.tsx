...
const coreStartMinute = 7 * 60;
const coreEndMinute = 20 * 60;
...
export function BookingScheduleView({
  ...
  const coreTop = (coreStartMinute - visibleRange.startMinute) * pixelsPerMinute;
  const coreHeight = (coreEndMinute - coreStartMinute) * pixelsPerMinute;
  ...
  return <section aria-label="Booking schedule" className="rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="overflow-x-auto">
      <div className="min-w-max" style={{ minWidth: `calc(64px + ${Math.max(1, renderColumns.length)} * ${minimumColumnWidth}px)` }}>
        <div className="grid border-b border-slate-200 bg-white" style={{ gridTemplateColumns: templateColumns }}>
          <div className="sticky left-0 z-30 flex min-h-16 items-end justify-end border-r border-slate-200 bg-white px-2 pb-2 text-[10px] font-black uppercase tracking-wide text-slate-400">{timezone.split('/').pop()?.replaceAll('_', ' ')}</div>
          {renderColumns.map(column => {
            const isToday = column.day === today;
            const isSelected = column.day === selectedDay;
            return <header key={column.key} className={`min-h-16 border-r px-3 py-2 transition ${isSelected ? 'border-indigo-300 bg-indigo-100/80 shadow-[inset_0_-3px_0_0_rgb(99_102_241)]' : isToday ? 'border-slate-200 bg-indigo-50/70' : 'border-slate-200 bg-white'}`}>
              <button type="button" onClick={() => onSelectDay?.(column.day)} aria-pressed={isSelected} className="w-full text-left">
                <span className="flex items-center justify-between gap-2">
                  <span className={`text-xs font-black uppercase tracking-wide ${isSelected || isToday ? 'text-indigo-700' : 'text-slate-500'}`}>{column.dayLabel}</span>
                  {isSelected && <span className="rounded-full bg-indigo-600 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-white">Selected</span>}
                </span>
                ...
                {isSelected && <span className="mt-1 block text-[10px] font-bold text-indigo-700">07:00–20:00 focus</span>}
              </button>
            </header>;
          })}
        </div>
        ...
        <div className="grid" style={{ gridTemplateColumns: templateColumns }}>
          <aside className="sticky left-0 z-20 border-r border-slate-200 bg-white" style={{ height: gridHeight }} aria-label="Calendar times">
            <div ref={focusAnchor} aria-hidden="true" className="absolute inset-x-0 h-px" style={{ top: coreTop }} />
          ...
          {renderColumns.map(column => {
            ...
            return <div
              ...
              className={`relative border-r ${isSelected ? 'border-indigo-300 bg-indigo-50/30' : column.day === today ? 'border-slate-200 bg-indigo-50/20' : 'border-slate-200 bg-white'}`}
              style={{
                height: gridHeight,
            ...
            >
              <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 bg-slate-100/80" style={{ height: coreTop }} />
              <div aria-label={isSelected ? 'Selected day focused availability hours' : undefined} className={`pointer-events-none absolute inset-x-0 ${isSelected ? 'bg-indigo-100/35 ring-2 ring-inset ring-indigo-200/70' : 'bg-transparent'}`} style={{ top: coreTop, height: coreHeight }} />
              <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 bg-slate-100/80" style={{ top: coreTop + coreHeight, bottom: 0 }} />
