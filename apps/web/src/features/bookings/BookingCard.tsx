...
export function BookingCard({ booking, density, onOpen }: BookingCardProps) {
  return <button type="button" onClick={() => onOpen(booking)} aria-label={`${localTime(booking.startTime, booking.timezone)} ${booking.customer.name}, ${booking.service.name}, ${booking.status.toLowerCase()}`} className={`w-full rounded-xl border-l-4 border border-slate-200 bg-white text-left shadow-xs transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 ${density === 'compact' ? 'p-2' : 'p-3'}`}>
    <div className="flex items-start justify-between gap-2"><div><p className="font-mono text-xs font-black text-slate-950">{localTime(booking.startTime, booking.timezone)}–{localTime(booking.endTime, booking.timezone)}</p><p className="mt-0.5 truncate text-sm font-black text-slate-900">{booking.customer.name}</p></div><BookingStatusBadge status={booking.status} compact /></div>
    {density !== 'compact' && <p className="mt-2 truncate text-xs font-semibold text-slate-600">{booking.service.name} · {booking.staff.name}</p>}
