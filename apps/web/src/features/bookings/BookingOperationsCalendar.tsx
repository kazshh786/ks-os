  ...
  return <main className="space-y-4" aria-busy={loading}>
    <header className="rounded-3xl border border-slate-200 bg-white p-4 shadow-xs">
      <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-start">
        <div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-600">Booking operations</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">Booking calendar</h1><p className="mt-1 break-words text-sm text-slate-500">{rangeLabel(range.from, range.to)} · {activeTenant.timezone}</p></div>
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          <button onClick={() => setCreateOpen(true)} className="col-span-2 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-black text-white shadow-sm sm:col-auto"><Plus className="h-4 w-4" />Create booking</button>
          <button onClick={() => setWalkInOpen(true)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm font-black text-indigo-800 sm:px-4"><ConciergeBell className="h-4 w-4" />Add walk-in</button>
          <button onClick={() => setBlockOpen(true)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-black sm:px-4"><Clock3 className="h-4 w-4" />Block time</button>
          <div className="relative col-span-2 sm:col-auto"><button onClick={() => setPublicMenu(value => !value)} aria-expanded={publicMenu} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-black"><Share2 className="h-4 w-4" />Public booking page</button>{publicMenu && <div className="absolute inset-x-0 z-20 mt-2 rounded-xl border bg-white p-2 shadow-xl sm:left-auto sm:right-0 sm:w-56"><button onClick={() => void publicPage('open')} className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-semibold hover:bg-slate-50"><ExternalLink className="h-4 w-4" />Open booking page</button><button onClick={() => void publicPage('copy')} className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-semibold hover:bg-slate-50"><Copy className="h-4 w-4" />Copy booking link</button><Link to="/app/settings/booking-page" className="flex min-h-11 items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold hover:bg-slate-50"><Settings2 className="h-4 w-4" />Page settings</Link></div>}</div>
          <button onClick={() => void exportCsv()} title="Export this filtered range as CSV" className="col-span-2 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-bold sm:col-auto"><Download className="h-4 w-4" />Export</button>
        </div>
      </div>
      <div className="mt-5 flex flex-col gap-3 border-t pt-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="grid grid-cols-[44px_auto_44px_minmax(0,1fr)] items-center gap-1"><button onClick={() => setAnchor(moveCalendarAnchor(anchor, view, -1))} aria-label="Previous calendar period" className="grid h-11 w-11 place-items-center rounded-lg border"><ChevronLeft className="h-4 w-4" /></button><button onClick={() => setAnchor(new Date())} className="min-h-11 rounded-lg border px-3 py-2 text-xs font-black">Today</button><button onClick={() => setAnchor(moveCalendarAnchor(anchor, view, 1))} aria-label="Next calendar period" className="grid h-11 w-11 place-items-center rounded-lg border"><ChevronRight className="h-4 w-4" /></button><input type="date" aria-label="Calendar date" value={dateValue} onChange={event => updateParams({ date: event.target.value })} className="min-h-11 min-w-0 w-full rounded-lg border p-2 text-xs font-bold" /></div>
        <div className="grid grid-cols-2 items-center gap-2 sm:flex sm:flex-wrap"><select aria-label="Calendar view" value={view} onChange={event => changeView(event.target.value as CalendarView)} className="min-h-11 w-full rounded-lg border bg-white px-3 text-xs font-bold">{calendarViews.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><select aria-label="Display density" value={density} onChange={event => changeDensity(event.target.value as typeof density)} className="min-h-11 w-full rounded-lg border bg-white px-3 text-xs font-bold"><option value="compact">Compact</option><option value="comfortable">Comfortable</option><option value="detailed">Detailed</option></select><Link to={view === 'agenda' ? '/app/calendar' : '/app/bookings'} className="col-span-2 inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border px-3 text-xs font-bold sm:col-auto"><CalendarDays className="h-4 w-4" />{view === 'agenda' ? 'Calendar view' : 'List view'}</Link></div>
      </div>
      <form onSubmit={event => { event.preventDefault(); updateParams({ search: searchValue || null }); }} className="mt-4 md:hidden">
        <label className="relative block"><span className="sr-only">Search bookings</span><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><input value={searchValue} onChange={event => setSearchValue(event.target.value)} placeholder="Customer, email, phone, reference…" className="min-h-11 w-full rounded-xl border pl-9 pr-3" /></label>
        <div className="mt-2 grid grid-cols-2 gap-2"><button type="submit" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 text-sm font-black text-white"><Search className="h-4 w-4" />Search</button><button type="button" onClick={() => setFiltersOpen(value => !value)} aria-expanded={filtersOpen} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-black"><Filter className="h-4 w-4" />Filters{filterCount > 0 ? ` (${filterCount})` : ''}</button></div>
        {filtersOpen && <div className="mt-3 grid gap-2 rounded-2xl border bg-slate-50 p-3"><select aria-label="Filter by staff" value={params.get('staff') || ''} onChange={event => updateParams({ staff: event.target.value || null })} className="min-h-11 w-full rounded-xl border bg-white px-3 font-bold"><option value="">All staff</option>{staff.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</select><select aria-label="Filter by service" value={params.get('service') || ''} onChange={event => updateParams({ service: event.target.value || null })} className="min-h-11 w-full rounded-xl border bg-white px-3 font-bold"><option value="">All services</option>{services.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}</select><select aria-label="Filter by location" value={params.get('location') || ''} onChange={event => updateParams({ location: event.target.value || null })} className="min-h-11 w-full rounded-xl border bg-white px-3 font-bold"><option value="">All locations</option>{locations.map(location => <option key={location.id} value={location.id}>{location.name}</option>)}</select><select aria-label="Filter by booking status" value={params.get('status') || ''} onChange={event => updateParams({ status: event.target.value || null })} className="min-h-11 w-full rounded-xl border bg-white px-3 font-bold"><option value="">All statuses</option>{Object.entries(bookingStatusDisplay).map(([status, display]) => <option key={status} value={status}>{display.label}</option>)}</select><select aria-label="Filter by payment status" value={params.get('payment') || ''} onChange={event => updateParams({ payment: event.target.value || null })} className="min-h-11 w-full rounded-xl border bg-white px-3 font-bold"><option value="">All payments</option><option value="NOT_REQUIRED">Not required</option><option value="PENDING">Pending</option><option value="PARTIALLY_PAID">Partially paid</option><option value="COMPLETED">Paid</option><option value="FAILED">Failed</option></select><button type="button" onClick={() => setFiltersOpen(false)} className="min-h-11 rounded-xl bg-white px-3 text-sm font-black text-indigo-700">Done</button></div>}
      </form>
      <form onSubmit={event => { event.preventDefault(); updateParams({ search: searchValue || null }); }} className="mt-4 hidden gap-2 md:grid md:grid-cols-2 xl:grid-cols-[minmax(240px,1fr)_repeat(6,minmax(120px,auto))]">
        <label className="relative"><span className="sr-only">Search bookings</span><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><input value={searchValue} onChange={event => setSearchValue(event.target.value)} placeholder="Customer, email, phone, reference…" className="min-h-11 w-full rounded-lg border pl-9 pr-3 text-sm" /></label>
        <select aria-label="Filter by staff" value={params.get('staff') || ''} onChange={event => updateParams({ staff: event.target.value || null })} className="min-h-11 rounded-lg border bg-white px-2 text-xs font-bold"><option value="">All staff</option>{staff.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</select>
        <select aria-label="Filter by service" value={params.get('service') || ''} onChange={event => updateParams({ service: event.target.value || null })} className="min-h-11 rounded-lg border bg-white px-2 text-xs font-bold"><option value="">All services</option>{services.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}</select>
        <select aria-label="Filter by location" value={params.get('location') || ''} onChange={event => updateParams({ location: event.target.value || null })} className="min-h-11 rounded-lg border bg-white px-2 text-xs font-bold"><option value="">All locations</option>{locations.map(location => <option key={location.id} value={location.id}>{location.name}</option>)}</select>
        <select aria-label="Filter by booking status" value={params.get('status') || ''} onChange={event => updateParams({ status: event.target.value || null })} className="min-h-11 rounded-lg border bg-white px-2 text-xs font-bold"><option value="">All statuses</option>{Object.entries(bookingStatusDisplay).map(([status, display]) => <option key={status} value={status}>{display.label}</option>)}</select>
        <select aria-label="Filter by payment status" value={params.get('payment') || ''} onChange={event => updateParams({ payment: event.target.value || null })} className="min-h-11 rounded-lg border bg-white px-2 text-xs font-bold"><option value="">All payments</option><option value="NOT_REQUIRED">Not required</option><option value="PENDING">Pending</option><option value="PARTIALLY_PAID">Partially paid</option><option value="COMPLETED">Paid</option><option value="FAILED">Failed</option></select>
        <button type="submit" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-slate-900 px-3 py-2 text-xs font-black text-white"><Filter className="h-4 w-4" />Apply</button>
      </form>
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs"><div className="flex flex-wrap items-center gap-2"><button onClick={() => updateParams({ attention: params.get('attention') === 'true' ? null : 'true' })} aria-pressed={params.get('attention') === 'true'} className={`inline-flex min-h-11 items-center gap-1 rounded-full border px-3 py-1.5 font-bold ${params.get('attention') === 'true' ? 'border-amber-300 bg-amber-50 text-amber-900' : ''}`}><AlertTriangle className="h-3.5 w-3.5" />Requires attention</button><span className="inline-flex min-h-11 items-center gap-1 text-slate-500"><SlidersHorizontal className="h-3.5 w-3.5" />{filterCount} active filters</span>{filterCount > 0 && <button onClick={() => { setSearchValue(''); setParams(new URLSearchParams({ view, date: dateValue }), { replace: true }); }} className="min-h-11 font-bold text-indigo-700">Clear all</button>}</div><button onClick={() => void load()} className="inline-flex min-h-11 items-center gap-1 font-bold text-slate-600"><RefreshCw className="h-3.5 w-3.5" />Refresh</button></div>
    </header>
