...
type Catalog = {
  ...
  page?: {
    ...
    bookingRules: { allowAnyStaff: boolean; maximumFutureDays?: number; enabledBookingChannels?: BookingChannel[]; serviceSelectionMode?: 'SINGLE' | 'MULTIPLE' | 'CUSTOM'; exclusiveServiceIds?: string[] };
  };
  tenant?: {
    name: string;
    timezone: string;
    ...
    businessAddress?: { name: string; address: string; postcode: string } | null;
...
export function PublicBookingFlow({ slug, preview = false, pageOverride, onBookingSuccess }: PublicBookingFlowProps) {
  ...
  const tenant = catalog?.tenant || { name: catalog?.tenantName || 'Business', timezone: 'Europe/London', currency: 'GBP', contactPhone: null, contactEmail: null, businessAddress: null, colors: { primary: '#111827', secondary: '#475569', accent: '#4f46e5' } };
  ...
  const selectedServices = useMemo(
    () => serviceIds.map(id => catalog?.services.find(item => item.id === id)).filter((item): item is Catalog['services'][number] => Boolean(item)),
    [catalog, serviceIds],
  );
  const service = selectedServices[0];
  const serviceNames = selectedServices.map(item => item.name).join(', ');
  const selectedDuration = selectedServices.reduce((total, item) => total + item.duration, 0);
  const selectedPrice = selectedServices.reduce((total, item) => total + item.price, 0);
  const selectedRequiresDeposit = selectedServices.some(item => item.requiresDeposit);
  const eligibleStaff = useMemo(
    () => catalog?.staff.filter(member => !member.serviceIds?.length || serviceIds.every(id => member.serviceIds!.includes(id))) || [],
    [catalog, serviceIds],
  );
  ...
  const businessAddress = [tenant.businessAddress?.address, tenant.businessAddress?.postcode].filter(Boolean).join(', ');
  const inShopAddress = location ? [location.address, location.postcode].filter(Boolean).join(', ') : businessAddress;
  const inShopLocationLabel = location
    ? [location.name, inShopAddress].filter(Boolean).join(' · ')
  ...
  const effectivePaymentMode = selectedRequiresDeposit ? 'deposit_required' : page?.paymentSettings.mode === 'FULL' ? 'pay_now' : page?.paymentSettings.mode === 'DEPOSIT' ? 'deposit_required' : page?.paymentSettings.mode === 'CUSTOMER_CHOICE' ? paymentChoice : 'pay_later';
  ...
  const quotedPrice = slot?.price ?? selectedPrice;
  ...
  const calendarHref = confirmation && selectedServices.length ? `data:text/calendar;charset=utf-8,${encodeURIComponent(['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//KS OS//Booking//EN', 'BEGIN:VEVENT', `UID:${confirmation.booking.reference}@ks-os`, `DTSTAMP:${calendarTimestamp(new Date().toISOString())}`, `DTSTART:${calendarTimestamp(confirmation.booking.startTime)}`, `DTEND:${calendarTimestamp(confirmation.booking.endTime)}`, `SUMMARY:${calendarText(`${serviceNames} at ${tenant.name}`)}`, `LOCATION:${calendarText(bookingChannel === 'mobile' ? `${addressLine1}, ${addressCity}, ${addressPostcode}` : inShopAddress)}`, `DESCRIPTION:${calendarText(`Booking reference ${confirmation.booking.reference}`)}`, 'END:VEVENT', 'END:VCALENDAR'].join('\r\n'))}` : '';
  ...
  useEffect(() => {
    ...
    provider.getPublicCatalog(slug).then((data: Catalog) => {
      ...
      setServiceIds(preselectedService ? [preselectedService.id] : []);
  ...
  const chooseService = (nextServiceId: string) => {
    releaseCurrentHold();
    const mode = page?.bookingRules.serviceSelectionMode || 'SINGLE';
    const exclusive = new Set(page?.bookingRules.exclusiveServiceIds || []);
    let nextIds: string[];
    if (mode === 'SINGLE') nextIds = [nextServiceId];
    else if (mode === 'CUSTOM' && exclusive.has(nextServiceId)) nextIds = serviceIds.length === 1 && serviceIds[0] === nextServiceId ? [] : [nextServiceId];
    else {
      const combinableIds = mode === 'CUSTOM' ? serviceIds.filter(id => !exclusive.has(id)) : serviceIds;
      nextIds = combinableIds.includes(nextServiceId)
        ? combinableIds.filter(id => id !== nextServiceId)
        : [...combinableIds, nextServiceId].slice(0, 10);
    }
    setServiceIds(nextIds);
    const nextEligibleStaff = catalog?.staff.filter(member => !member.serviceIds?.length || nextIds.every(id => member.serviceIds!.includes(id))) || [];
    if (page?.bookingRules.allowAnyStaff !== false) setStaffId('any');
    else if (!nextEligibleStaff.some(member => member.id === staffId)) setStaffId(nextEligibleStaff.find(member => member.accountRole === 'owner')?.id || nextEligibleStaff[0]?.id || '');
    setPaymentChoice(nextIds.some(id => catalog?.services.find(item => item.id === id)?.requiresDeposit) ? 'deposit_required' : 'pay_later');
    track('SERVICE_SELECTED', { serviceId: nextServiceId, locationId: locationId || undefined });
  };
  const chooseSlot = async (nextSlot: Slot) => {
    setError('');
    if (hold) await provider.releaseBookingHold(slug, hold.id, hold.token).catch(() => undefined);
    try {
      const nextHold = preview ? { id: crypto.randomUUID(), token: 'preview-token-preview-token-preview-token', startTime: nextSlot.start, endTime: nextSlot.end, expiresAt: new Date(Date.now() + 10 * 60_000).toISOString(), remainingSeconds: 600 } : await provider.createBookingHold(slug, { serviceId, serviceIds, staffId: nextSlot.staffId, locationId: locationId || null, startTime: nextSlot.start, bookingChannel, idempotencyKey: crypto.randomUUID() });
  ...
  const canContinueFromService = Boolean(serviceIds.length && staffId && eligibleStaff.length > 0);
  ...
  return <main className="mx-auto w-full max-w-6xl min-w-0" style={themeVariables}><div className="booking-workspace overflow-hidden rounded-2xl border border-white/90 bg-white shadow-2xl shadow-slate-300/50 ring-1 ring-slate-200/70 sm:rounded-[2rem]">
    ...
    <div className="booking-workspace-body grid lg:grid-cols-[minmax(0,1fr)_320px]"><section ref={stepColumnRef} className="booking-step-column min-w-0 px-5 py-5 sm:px-7">
      ...
      {selectedServices.length > 0 && <div className="mb-5 flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-3 lg:hidden"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-950">{serviceNames}</p><p className="mt-1 text-xs text-slate-500">{slot ? dateTime(slot.start, 'medium') : `${selectedDuration} minutes`}</p></div><p className="shrink-0 text-sm font-black text-slate-950">{currency(quotedPrice)}</p></div>}
      ...
      {step === 0 && <div className="booking-step-content"><SectionHeading eyebrow="Step 1" title={`Book with ${tenant.name}`} description="Choose what you need. You’ll see live availability before entering any personal details." />
        ...
        {bookingChannel === 'in_shop' && businessAddress && (!catalog.locations || catalog.locations.length <= 1) && <div className="flex gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4"><MapPin className="mt-0.5 h-5 w-5 shrink-0" style={{ color: primary }} /><div><p className="font-black text-slate-950">{tenant.businessAddress?.name || tenant.name}</p><p className="mt-1 text-sm leading-6 text-slate-600">{businessAddress}</p></div></div>}
        ...
        <fieldset><legend className="text-sm font-black text-slate-950">{page?.bookingRules.serviceSelectionMode === 'SINGLE' ? 'Choose a service' : 'Choose one or more services'}</legend>{page?.bookingRules.serviceSelectionMode !== 'SINGLE' && <p className="mt-1 text-xs leading-5 text-slate-500">Select every service you want in this appointment. Your total time and price update automatically.</p>}<div className="mt-3 space-y-5">{serviceGroups.map(([category, items]) => <section key={category} aria-labelledby={`service-category-${category.replace(/\W+/g, '-').toLowerCase()}`}><div className="booking-service-category-heading"><h3 id={`service-category-${category.replace(/\W+/g, '-').toLowerCase()}`}>{category}</h3><span>{items.length} {items.length === 1 ? 'service' : 'services'}</span></div><div className="booking-service-list">{items.map(item => { const discounted = Boolean(item.discount && item.basePrice && item.basePrice > item.price); return <ChoiceCard key={item.id} selected={serviceIds.includes(item.id)} onClick={() => chooseService(item.id)} primary={primary} ariaLabel={`${item.name}, ${item.duration} minutes, ${currency(item.price)}`} className="booking-service-choice"><div className="booking-service-choice__top"><div className="min-w-0"><p>{item.name}</p><small>{item.description || 'Choose this service to see available appointment times.'}</small></div><div className="booking-service-choice__price">{discounted && <s>{currency(item.basePrice || item.price)}</s>}<strong>{currency(item.price)}</strong></div></div><div className="booking-service-choice__meta"><span><Clock3 aria-hidden="true" />{item.duration} min</span>{item.requiresDeposit && <span className="is-warning">Deposit required</span>}{page?.bookingRules.serviceSelectionMode === 'CUSTOM' && page.bookingRules.exclusiveServiceIds?.includes(item.id) && <span className="is-warning">Booked on its own</span>}{discounted && <span className="is-success">Discount applied</span>}</div></ChoiceCard>; })}</div></section>)}</div></fieldset>
        {serviceIds.length > 0 && <fieldset><legend className="text-sm font-black text-slate-950">Choose who you book with</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">{page?.bookingRules.allowAnyStaff !== false && <ChoiceCard selected={staffId === 'any'} onClick={() => { releaseCurrentHold(); setStaffId('any'); }} primary={primary} ariaLabel="Anyone available. Show the earliest times across the eligible team."><div className="grid h-10 w-10 place-items-center rounded-full" style={{ backgroundColor: rgba(primary, 0.1) }}><Sparkles className="h-5 w-5" style={{ color: primary }} /></div><p className="mt-3 pr-7 font-black text-slate-950">Anyone available</p><p className="mt-1 text-xs leading-5 text-slate-500">Best for finding the earliest appointment.</p></ChoiceCard>}{eligibleStaff.map(member => <ChoiceCard key={member.id} selected={staffId === member.id} onClick={() => { releaseCurrentHold(); setStaffId(member.id); track('STAFF_SELECTED', { serviceId, staffId: member.id }); }} primary={primary} ariaLabel={`${member.name}, ${member.role || 'Team member'}`}><div className="flex items-center gap-3 pr-7">{member.imageUrl ? <img src={member.imageUrl} alt="" className="h-10 w-10 rounded-full object-cover" /> : <div className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 text-xs font-black text-slate-700">{initials(member.name)}</div>}<div><p className="font-black text-slate-950">{member.name}{member.accountRole === 'owner' ? ' · Owner' : ''}</p><p className="mt-0.5 text-xs text-slate-500">{member.role || 'Team member'}</p></div></div></ChoiceCard>)}</div></fieldset>}
