...
export function PublicBookingFlow({ slug, preview = false, pageOverride, onBookingSuccess }: PublicBookingFlowProps) {
  ...
  if (loading) return <div className="mx-auto max-w-3xl rounded-2xl bg-white p-6 text-center font-bold text-slate-500 sm:rounded-3xl sm:p-12" aria-live="polite">Loading booking page…</div>;
  if (loadError) return <div role="alert" className="mx-auto max-w-2xl rounded-2xl border border-rose-200 bg-white p-6 text-center sm:rounded-3xl sm:p-12"><h1 className="text-xl font-black">Booking page unavailable</h1><p className="mt-2 break-words text-slate-600">{loadError}</p></div>;
  if (!catalog || catalog.services.length === 0) return <div className="mx-auto max-w-2xl rounded-2xl border bg-white p-6 text-center sm:rounded-3xl sm:p-12"><h1 className="text-xl font-black">No active services available</h1><p className="mt-2 text-slate-600">This business is not accepting online bookings yet.</p></div>;

  if (confirmation) return <main className="mx-auto max-w-2xl rounded-3xl border bg-white p-6 shadow-sm sm:p-10">
    <CheckCircle2 className="mx-auto h-14 w-14 text-emerald-600" />
    <h1 className="mt-4 text-center text-2xl font-black sm:text-3xl">Booking confirmed</h1>
    <p className="mt-2 break-words text-center text-slate-600">A confirmation and secure reschedule or cancellation link will be sent to <span className="break-all font-semibold">{email}</span>.</p>
    <dl className="mt-8 grid gap-3 rounded-2xl bg-slate-50 p-5 text-sm">
      <div className="grid gap-1 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Reference</dt><dd className="break-all font-mono font-black sm:text-right">{confirmation.booking.reference}</dd></div>
      <div className="grid gap-1 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Service</dt><dd className="break-words font-bold sm:text-right">{service?.name}</dd></div>
      <div className="grid gap-1 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Team member</dt><dd className="break-words font-bold sm:text-right">{selectedStaff?.name}</dd></div>
      <div className="grid gap-1 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Location</dt><dd className="break-words font-bold sm:text-right">{bookingChannel === 'mobile' ? `${addressLine1}, ${addressCity}, ${addressPostcode}` : location?.name || 'Primary location'}</dd></div>
      <div className="grid gap-1 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Date and time</dt><dd className="break-words font-bold sm:text-right">{new Intl.DateTimeFormat('en-GB', { timeZone: tenant.timezone, dateStyle: 'full', timeStyle: 'short' }).format(new Date(confirmation.booking.startTime))}</dd></div>
      <div className="grid gap-1 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Payment</dt><dd className="break-words font-bold sm:text-right">{confirmation.payment.required ? `${confirmation.payment.status} · ${new Intl.NumberFormat('en-GB', { style: 'currency', currency: tenant.currency }).format(confirmation.payment.amount / 100)}` : 'Pay later / not required'}</dd></div>
    </dl>
  ...
  return <main className="mx-auto w-full max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl" style={{ '--booking-primary': page?.theme.primaryColor || tenant.colors.primary } as React.CSSProperties}>
    ...
    <header className="p-5 text-white sm:p-8" style={{ backgroundColor: page?.theme.primaryColor || tenant.colors.primary }}><div className="flex items-start gap-3 sm:items-center sm:gap-4">{page?.logoUrl && <img src={page.logoUrl} alt={`${tenant.name} logo`} className="h-12 w-12 shrink-0 rounded-xl bg-white object-contain p-1 sm:h-14 sm:w-14" />}<div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.16em] opacity-80 sm:text-xs sm:tracking-[0.18em]">Secure online booking</p><h1 className="mt-1 break-words text-2xl font-black sm:text-3xl">{page?.title || tenant.name}</h1><p className="mt-1 max-w-2xl break-words text-sm opacity-85">{page?.description || `Choose a service and time with ${tenant.name}.`}</p></div></div></header>
    <div className="grid lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      {step === 0 && <div className="space-y-7">
        ...
        <fieldset>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">{catalog.services.map(item => {
            ...
            return <button key={item.id} onClick={() => {
              ...
              <div className="grid gap-1 sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-3"><p className="break-words font-black">{item.name}</p><p className="font-black sm:text-right">{discounted && <span className="mr-2 text-xs font-semibold text-slate-400 line-through">{new Intl.NumberFormat('en-GB', { style: 'currency', currency: tenant.currency }).format((item.basePrice || item.price) / 100)}</span>}{new Intl.NumberFormat('en-GB', { style: 'currency', currency: tenant.currency }).format(item.price / 100)}</p></div>
              <p className="mt-1 break-words text-sm text-slate-500">{item.description || 'Contact the business if you need more information about this service.'}</p>
        ...
        <div className="flex justify-end"><button disabled={!serviceId || !staffId || eligibleStaff.length === 0} onClick={() => { setStep(1); track('BOOKING_STARTED', { serviceId, locationId: locationId || undefined }); }} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 font-black text-white disabled:opacity-40 sm:w-auto">Choose a time<ChevronRight className="h-4 w-4" /></button></div>
      </div>}
        {step === 1 && <div><h2 className="text-2xl font-black">Choose a date and time</h2><p className="mt-1 break-words text-sm text-slate-500">Times are shown in {tenant.timezone}. Your selection will be reserved briefly while you finish.</p><div className="mt-5 flex snap-x gap-2 overflow-x-auto pb-3">{dates.map(item => { const value = format(item, 'yyyy-MM-dd'); return <button key={value} onClick={() => { setDate(value); track('DATE_SELECTED', { serviceId, staffId: staffId === 'any' ? undefined : staffId }); }} aria-pressed={date === value} className={`min-w-20 snap-start rounded-xl border p-3 text-center ${date === value ? 'border-indigo-500 bg-indigo-50' : ''}`}><span className="block text-xs font-bold uppercase text-slate-500">{format(item, 'EEE')}</span><span className="mt-1 block text-xl font-black">{format(item, 'd')}</span><span className="block text-xs">{format(item, 'MMM')}</span></button>; })}</div>{slotsLoading ? <p className="mt-8 text-center text-sm font-bold text-slate-500">Checking live availability…</p> : slots.length ? <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">{slots.map(item => <button key={`${item.start}-${item.staffId}`} onClick={() => void chooseSlot(item)} className="min-h-12 rounded-xl border px-2 text-sm font-black hover:border-indigo-500 hover:bg-indigo-50">{new Intl.DateTimeFormat('en-GB', { timeZone: tenant.timezone, hour: '2-digit', minute: '2-digit' }).format(new Date(item.start))}<span className="block break-words text-[10px] font-semibold text-slate-500">{item.staffName}</span></button>)}</div> : <div className="mt-8 rounded-2xl border border-dashed p-6 text-center sm:p-8"><CalendarDays className="mx-auto h-8 w-8 text-slate-400" /><h3 className="mt-3 font-black">No availability on this date</h3><p className="mt-1 text-sm text-slate-500">Try another date or choose “Any available” to see more options.</p></div>}<div className="mt-8"><button onClick={() => setStep(0)} className="inline-flex min-h-11 items-center gap-1 rounded-lg border px-4 py-2 text-sm font-bold"><ChevronLeft className="h-4 w-4" />Back</button></div></div>}
        {step === 2 && <form onSubmit={event => { event.preventDefault(); setStep(3); }}>
          ...
          <div className="mt-8 grid grid-cols-1 gap-2 min-[360px]:grid-cols-2"><button type="button" onClick={() => setStep(1)} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-lg border px-4 py-2 text-sm font-bold"><ChevronLeft className="h-4 w-4" />Back</button><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 font-black text-white">Review booking<ChevronRight className="h-4 w-4" /></button></div>
        </form>}
        {step === 3 && <div>
          ...
          <dl className="mt-5 divide-y rounded-2xl border p-5 text-sm">
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Service</dt><dd className="break-words font-black sm:text-right">{service?.name}</dd></div>
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Team member</dt><dd className="break-words font-black sm:text-right">{selectedStaff?.name}</dd></div>
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>{bookingChannel === 'mobile' ? 'Appointment address' : 'Location'}</dt><dd className="break-words font-black sm:text-right">{bookingChannel === 'mobile' ? [addressLine1, addressLine2, addressCity, addressPostcode].filter(Boolean).join(', ') : location?.name || 'Primary location'}</dd></div>
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Date and time</dt><dd className="break-words font-black sm:text-right">{slot && new Intl.DateTimeFormat('en-GB', { timeZone: tenant.timezone, dateStyle: 'full', timeStyle: 'short' }).format(new Date(slot.start))}</dd></div>
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Duration</dt><dd className="font-black sm:text-right">{slot?.duration || service?.duration} minutes</dd></div>
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>Total</dt><dd className="font-black sm:text-right">{new Intl.NumberFormat('en-GB', { style: 'currency', currency: tenant.currency }).format(quotedPrice / 100)}</dd></div>
            <div className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:gap-4"><dt>{effectivePaymentMode === 'deposit_required' ? `Deposit due now${depositPercentage > 0 ? ` (${depositPercentage}%)` : ''}` : 'Payment'}</dt><dd className="font-black sm:text-right">{amountDueNow > 0 ? new Intl.NumberFormat('en-GB', { style: 'currency', currency: tenant.currency }).format(amountDueNow / 100) : 'Pay later'}</dd></div>
          </dl>
          ...
          <div className="mt-8 grid grid-cols-1 gap-2 min-[360px]:grid-cols-2"><button onClick={() => setStep(2)} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-lg border px-4 py-2 text-sm font-bold"><ChevronLeft className="h-4 w-4" />Back</button><button onClick={() => void submit()} disabled={submitting || remaining <= 0} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 font-black text-white disabled:opacity-40">{submitting ? 'Confirming safely…' : effectivePaymentMode !== 'pay_later' ? 'Continue to payment' : 'Confirm booking'}<Check className="h-4 w-4" /></button></div>
        </div>}
