...
export function PublicBookingFlow({ slug, preview = false, pageOverride, onBookingSuccess }: PublicBookingFlowProps) {
  ...
  const resetStepScroll = () => window.requestAnimationFrame(() => stepColumnRef.current?.scrollTo({ top: 0, behavior: 'smooth' }));
  ...
  return <main className="mx-auto w-full max-w-6xl" style={themeVariables}><div className="booking-workspace overflow-hidden rounded-[2rem] border border-white/90 bg-white shadow-2xl shadow-slate-300/50 ring-1 ring-slate-200/70">
    ...
    <div className="booking-workspace-body grid lg:grid-cols-[minmax(0,1fr)_320px]"><section ref={stepColumnRef} className="booking-step-column min-w-0 px-5 py-5 sm:px-7">
      ...
      {step === 3 && <div className="booking-step-content"><SectionHeading eyebrow="Step 4" title="Review and confirm" description="Check the details below. Nothing is booked or charged until you confirm." />{page?.paymentSettings.mode === 'CUSTOMER_CHOICE' && !service?.requiresDeposit && <fieldset className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4 sm:p-5"><legend className="px-2 text-sm font-black text-slate-950">How would you like to pay?</legend><div className="grid gap-3 sm:grid-cols-2"><ChoiceCard selected={paymentChoice === 'pay_now'} onClick={() => setPaymentChoice('pay_now')} primary={primary} ariaLabel="Pay now securely online"><CreditCard className="h-5 w-5" style={{ color: primary }} /><p className="mt-3 pr-7 font-black text-slate-950">Pay now</p><p className="mt-1 text-xs leading-5 text-slate-500">Complete payment using Stripe’s secure checkout.</p></ChoiceCard><ChoiceCard selected={paymentChoice === 'pay_later'} onClick={() => setPaymentChoice('pay_later')} primary={primary} ariaLabel="Pay later directly to the business"><Wallet className="h-5 w-5" style={{ color: primary }} /><p className="mt-3 pr-7 font-black text-slate-950">Pay later</p><p className="mt-1 text-xs leading-5 text-slate-500">Pay directly to the business.</p></ChoiceCard>{amountDueNow > 0 && <ChoiceCard selected={paymentChoice === 'deposit_required'} onClick={() => setPaymentChoice('deposit_required')} primary={primary} ariaLabel={'Pay a ' + depositChoiceLabel + ' deposit'} className="sm:col-span-2"><ShieldCheck className="h-5 w-5" style={{ color: primary }} /><p className="mt-3 pr-7 font-black text-slate-950">Pay a {depositChoiceLabel} deposit</p><p className="mt-1 text-xs leading-5 text-slate-500">Secure the booking now and pay the balance later.</p></ChoiceCard>}</div></fieldset>}
        <dl className="mt-5 divide-y divide-slate-100 overflow-hidden rounded-3xl border border-slate-200 bg-white text-sm"><div className="flex justify-between gap-4 px-5 py-3.5"><dt className="text-slate-500">Service</dt><dd className="text-right font-black text-slate-950">{service?.name}</dd></div><div className="flex justify-between gap-4 px-5 py-3.5"><dt className="text-slate-500">Team member</dt><dd className="text-right font-black text-slate-950">{selectedStaff?.name}</dd></div><div className="flex justify-between gap-4 px-5 py-3.5"><dt className="text-slate-500">{bookingChannel === 'mobile' ? 'Appointment address' : 'Location'}</dt><dd className="max-w-[65%] text-right font-black text-slate-950">{bookingChannel === 'mobile' ? [addressLine1, addressLine2, addressCity, addressPostcode].filter(Boolean).join(', ') : location?.name || 'Primary location'}</dd></div><div className="flex justify-between gap-4 px-5 py-3.5"><dt className="text-slate-500">Date and time</dt><dd className="max-w-[65%] text-right font-black text-slate-950">{slot && dateTime(slot.start)}</dd></div><div className="flex justify-between gap-4 px-5 py-3.5"><dt className="text-slate-500">Duration</dt><dd className="font-black text-slate-950">{slot?.duration || service?.duration} minutes</dd></div><div className="flex justify-between gap-4 px-5 py-3.5"><dt className="text-slate-500">Total</dt><dd className="font-black text-slate-950">{currency(quotedPrice)}</dd></div><div className="flex justify-between gap-4 bg-slate-50 px-5 py-3.5"><dt className="font-black text-slate-700">{effectivePaymentMode === 'deposit_required' ? `Deposit due now (${depositType === 'FIXED' ? 'fixed amount' : `${depositPercentage}%`})` : 'Payment due now'}</dt><dd className="font-black text-slate-950">{amountDueNow > 0 ? currency(amountDueNow) : 'Pay later'}</dd></div></dl>
