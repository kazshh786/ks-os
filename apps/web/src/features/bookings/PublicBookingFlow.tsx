...
type Catalog = {
  ...
  staff: Array<{ id: string; name: string; role?: string | null; imageUrl?: string | null; bio?: string | null; serviceIds?: string[] }>;
  ...
  return <main className="mx-auto w-full max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl" style={{ '--booking-primary': page?.theme.primaryColor || tenant.colors.primary } as React.CSSProperties}>
    ...
    <div className="grid lg:grid-cols-[minmax(0,1fr)_300px]">
      <section className="p-5 sm:p-8">
        ...
        {step === 0 && <div className="space-y-7">{Boolean(catalog.bookingChannels && catalog.bookingChannels.length > 1) && <fieldset><legend className="text-lg font-black">Where should the appointment happen?</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">{catalog.bookingChannels!.map(channel => <button key={channel.id} onClick={() => { setBookingChannel(channel.id); setSlot(null); setHold(null); }} aria-pressed={bookingChannel === channel.id} className={`rounded-xl border p-4 text-left ${bookingChannel === channel.id ? 'border-indigo-500 bg-indigo-50' : ''}`}><MapPin className="h-5 w-5 text-indigo-600" /><p className="mt-2 font-black">{channel.label}</p><p className="text-xs text-slate-500">{channel.id === 'mobile' ? 'The team travels to the address you provide.' : 'Visit the business at your chosen location.'}</p></button>)}</div></fieldset>}{Boolean(catalog.locations?.length) && <fieldset><legend className="text-lg font-black">Choose a location</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">{catalog.locations!.map(item => <button key={item.id} onClick={() => setLocationId(item.id)} aria-pressed={locationId === item.id} className={`rounded-xl border p-4 text-left ${locationId === item.id ? 'border-indigo-500 bg-indigo-50' : ''}`}><MapPin className="h-5 w-5 text-indigo-600" /><p className="mt-2 font-black">{item.name}</p><p className="text-xs text-slate-500">{item.address}, {item.postcode}</p></button>)}</div></fieldset>}<fieldset><legend className="text-lg font-black">Choose a service</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">{catalog.services.map(item => <button key={item.id} onClick={() => { setServiceId(item.id); setStaffId('any'); track('SERVICE_SELECTED', { serviceId: item.id, locationId: locationId || undefined }); }} aria-pressed={serviceId === item.id} className={`rounded-xl border p-4 text-left ${serviceId === item.id ? 'border-indigo-500 bg-indigo-50' : ''}`}><div className="flex justify-between gap-3"><p className="font-black">{item.name}</p><p className="font-black">{new Intl.NumberFormat('en-GB', { style: 'currency', currency: tenant.currency }).format(item.price / 100)}</p></div><p className="mt-1 text-sm text-slate-500">{item.description}</p><p className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-slate-600"><Clock3 className="h-3.5 w-3.5" />{item.duration} minutes</p></button>)}</div></fieldset>{serviceId && <fieldset><legend className="text-lg font-black">Choose a team member</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">{page?.bookingRules.allowAnyStaff !== false && <button onClick={() => setStaffId('any')} aria-pressed={staffId === 'any'} className={`rounded-xl border p-4 text-left ${staffId === 'any' ? 'border-indigo-500 bg-indigo-50' : ''}`}><UserRound className="h-5 w-5 text-indigo-600" /><p className="mt-2 font-black">Any available</p><p className="text-xs text-slate-500">Show the earliest times across the team.</p></button>}{eligibleStaff.map(member => <button key={member.id} onClick={() => { setStaffId(member.id); track('STAFF_SELECTED', { serviceId, staffId: member.id }); }} aria-pressed={staffId === member.id} className={`rounded-xl border p-4 text-left ${staffId === member.id ? 'border-indigo-500 bg-indigo-50' : ''}`}><p className="font-black">{member.name}</p><p className="text-xs text-slate-500">{member.role || 'Team member'}</p></button>)}</div></fieldset>}<div className="flex justify-end"><button disabled={!serviceId || (!staffId && eligibleStaff.length > 0)} onClick={() => { setStep(1); track('BOOKING_STARTED', { serviceId, locationId: locationId || undefined }); }} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 font-black text-white disabled:opacity-40">Choose a time<ChevronRight className="h-4 w-4" /></button></div></div>}
