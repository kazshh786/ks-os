  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/50 sm:items-center sm:p-6">
    <section role="dialog" aria-modal="true" aria-labelledby="block-time-title" className="w-full max-w-xl rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="flex justify-end gap-3 sm:col-span-2"><button type="button" onClick={onClose} className="rounded-xl border px-4 py-2.5 text-sm font-bold">Cancel</button><button disabled={saving || !staff.length} className="rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{saving ? 'Blocking time…' : 'Block time'}</button></div>
      </form>
    </section>
  </div>;
}
