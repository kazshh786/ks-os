...
const formatMoney = (value: number) => new Intl.NumberFormat('en-GB', { style: 'currency', currency: 'GBP' }).format(value / 100);
...
export function BookingQuickView({ booking, staff, initialReschedule = null, onClose, onChanged, onCheckout }: BookingQuickViewProps) {
  ...
  useEffect(() => {
    if (!booking) return;
    const proposedStart = initialReschedule?.startTime || booking.startTime;
  ...
  const proposedStart = fromZonedTime(`${date}T${time}:00`, booking.timezone).toISOString();
  const proposedLabel = formatDateTime(proposedStart);
        ...
        <div className="flex min-w-0 items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {breakdown && <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${breakdown.repeatCustomer ? 'bg-violet-100 text-violet-800' : 'bg-emerald-100 text-emerald-800'}`}>
                {breakdown.repeatCustomer ? <Repeat2 className="h-3 w-3" /> : <Sparkles className="h-3 w-3" />}{breakdown.repeatCustomer ? 'Repeat customer' : 'New customer'}
              </span>}
            </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 sm:p-5">
        ...
        {rescheduling && <form onSubmit={saveReschedule} className="mb-4 rounded-2xl border-2 border-indigo-300 bg-indigo-50 p-4 shadow-sm">
          <div className="flex items-start gap-3">
            <CalendarClock className="mt-0.5 h-5 w-5 shrink-0 text-indigo-700" />
            <div>
              <h3 className="font-black text-indigo-950">Are you sure you want to reschedule?</h3>
              <p className="mt-1 text-sm text-indigo-900">The customer will be notified after you confirm the new date and time.</p>
              {initialReschedule && <p className="mt-2 rounded-lg bg-white/70 px-3 py-2 text-xs font-bold text-indigo-950">Dragged to {initialReschedule.targetLabel}. Review the exact date, time and team member below before confirming.</p>}
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-2 rounded-xl border border-indigo-200 bg-white p-3 text-sm sm:grid-cols-[1fr_auto_1fr] sm:items-center">
            <div><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">Current</span><strong>{formatDateTime(booking.startTime)}</strong><span className="block text-xs text-slate-500">{booking.staff.name}</span></div>
            <span className="hidden text-indigo-500 sm:block">→</span>
            <div><span className="block text-[10px] font-black uppercase tracking-wide text-indigo-500">Proposed</span><strong className="text-indigo-950">{proposedLabel}</strong><span className="block text-xs text-indigo-700">{selectedStaffName}</span></div>
          </div>
          ...
          <div className="space-y-4 lg:col-span-2">
            <DetailCard title="Appointment details">
              <div className="grid gap-3 text-sm sm:grid-cols-2">
                ...
                <p className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 text-indigo-600" /><span><strong className="block text-slate-950">{record.location.name || (record.bookingChannel === 'mobile' ? 'Mobile appointment' : 'Primary location')}</strong><span className="text-slate-500">{humanize(record.bookingChannel)}</span></span></p>
                <p className="flex items-start gap-2"><CreditCard className="mt-0.5 h-4 w-4 text-indigo-600" /><span><strong className="block text-slate-950">{formatMoney(record.quotedAmount)}</strong><span className="text-slate-500">{humanize(record.paymentStatus)}</span></span></p>
              </div>
            </DetailCard>

            {breakdown && <DetailCard title="Customer history">
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                <Stat label="Completed visits" value={breakdown.completedVisits} />
                <Stat label="Total bookings" value={breakdown.totalBookings} />
                <Stat label="Upcoming" value={breakdown.upcomingBookings} />
                <Stat label="Loyalty points" value={breakdown.loyaltyPoints} />
              </div>
              <div className="mt-3 grid gap-3 rounded-xl bg-slate-50 p-3 text-sm sm:grid-cols-2">
                <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">First completed visit</span><strong>{formatDate(breakdown.firstVisitAt)}</strong></p>
                <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">Last completed visit</span><strong>{formatDate(breakdown.lastVisitAt)}</strong></p>
                <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">Cancellations</span><strong>{breakdown.cancellations}</strong></p>
                <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">No-shows</span><strong>{breakdown.noShows}</strong></p>
              </div>
            </DetailCard>}

            <DetailCard title="Booking forms and answers">
              {detail?.formResponses.length ? <div className="space-y-3">{detail.formResponses.map(form => <article key={form.assignmentId} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2"><div><h4 className="font-black text-slate-950">{form.formTitle}</h4><p className="text-xs text-slate-500">{form.submittedAt ? `Submitted ${formatDateTime(form.submittedAt)}` : humanize(form.status)}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${form.status === 'SUBMITTED' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{humanize(form.status)}</span></div>
                {form.answers.length ? <dl className="mt-3 grid gap-2 sm:grid-cols-2">{form.answers.map(answer => <div key={answer.key} className="rounded-lg border border-slate-200 bg-white p-3"><dt className="text-[10px] font-black uppercase tracking-wide text-slate-400">{answer.label}</dt><dd className="mt-1 whitespace-pre-wrap text-sm font-semibold text-slate-800">{answer.displayValue}</dd></div>)}</dl> : <p className="mt-3 text-sm text-slate-500">No submitted answers are available for this form yet.</p>}
              </article>)}</div> : <p className="text-sm text-slate-500">No booking forms are attached to this appointment.</p>}
            </DetailCard>

            {(record.notes || record.customerNotes || breakdown?.medicalNotes) && <div className="grid gap-4 sm:grid-cols-2">
              {record.notes && <DetailCard title="Internal notes"><p className="whitespace-pre-wrap text-sm text-slate-800">{record.notes}</p></DetailCard>}
              {record.customerNotes && <DetailCard title="Customer booking notes"><p className="whitespace-pre-wrap text-sm text-slate-800">{record.customerNotes}</p></DetailCard>}
              {breakdown?.medicalNotes && <DetailCard title="Medical notes" className="border-rose-200 bg-rose-50 sm:col-span-2"><p className="flex items-start gap-2 whitespace-pre-wrap text-sm text-rose-950"><HeartPulse className="mt-0.5 h-4 w-4 shrink-0" />{breakdown.medicalNotes}</p></DetailCard>}
            </div>}
          </div>
          ...
          <aside className="space-y-4">
            <DetailCard title="Customer contact">
              <div className="space-y-3 text-sm">
                <p className="flex items-center gap-2"><CircleUserRound className="h-4 w-4 text-indigo-600" /><strong>{record.customer.name}</strong></p>
                {record.customer.email ? <a href={`mailto:${record.customer.email}`} className="flex items-center gap-2 break-all font-semibold text-indigo-700 hover:underline"><Mail className="h-4 w-4 shrink-0" />{record.customer.email}</a> : <p className="flex items-center gap-2 text-slate-500"><Mail className="h-4 w-4" />No email</p>}
                {record.customer.phone ? <a href={`tel:${record.customer.phone}`} className="flex items-center gap-2 font-semibold text-indigo-700 hover:underline"><Phone className="h-4 w-4" />{record.customer.phone}</a> : <p className="flex items-center gap-2 text-slate-500"><Phone className="h-4 w-4" />No phone</p>}
                {breakdown && <p className="flex items-center gap-2 text-slate-600"><History className="h-4 w-4" />Customer since {formatDate(breakdown.memberSince)}</p>}
                {breakdown?.patchTestDate && <p className="flex items-center gap-2 text-slate-600"><CheckCircle2 className="h-4 w-4 text-emerald-600" />Patch test {formatDate(breakdown.patchTestDate)}</p>}
              </div>
            </DetailCard>

            <DetailCard title="Source and attribution">
              <div className="space-y-3 text-sm">
                <p className="flex items-start gap-2"><Megaphone className="mt-0.5 h-4 w-4 text-indigo-600" /><span><strong className="block text-slate-950">{sourceLabels[acquisition?.source || record.source] || humanize(acquisition?.source || record.source)}</strong><span className="text-slate-500">Booked {formatDateTime(acquisition?.bookedAt || record.createdAt)}</span></span></p>
                {acquisition?.medium && <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">Medium</span><strong>{acquisition.medium}</strong></p>}
                {acquisition?.campaign && <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">Campaign</span><strong>{acquisition.campaign}</strong></p>}
                {acquisition?.referrerHost && <p><span className="block text-[10px] font-black uppercase tracking-wide text-slate-400">Referrer</span><strong className="break-all">{acquisition.referrerHost}</strong></p>}
              </div>
            </DetailCard>

            <DetailCard title="Operational status">
              <div className="space-y-2 text-sm">
                <p className="flex items-center justify-between gap-3"><span className="text-slate-500">Payment</span><strong>{humanize(record.paymentStatus)}</strong></p>
                <p className="flex items-center justify-between gap-3"><span className="text-slate-500">Forms</span><strong>{humanize(record.intakeStatus)}</strong></p>
                <p className="flex items-center justify-between gap-3"><span className="text-slate-500">Channel</span><strong>{humanize(record.bookingChannel)}</strong></p>
              </div>
            </DetailCard>
