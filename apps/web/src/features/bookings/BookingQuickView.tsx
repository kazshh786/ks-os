...
export function BookingQuickView({ booking, staff, onClose, onChanged, onCheckout }: BookingQuickViewProps) {
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/30" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <aside ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="booking-quick-view-title" tabIndex={-1} className="ml-auto flex h-dvh w-full max-w-xl flex-col overflow-hidden bg-white shadow-2xl">
      <header className="z-10 flex shrink-0 items-start justify-between border-b bg-white p-4 pt-[max(1rem,env(safe-area-inset-top))] sm:p-5"><div className="min-w-0"><p className="break-all font-mono text-xs font-bold uppercase tracking-wider text-slate-500">{booking.reference}</p><h2 id="booking-quick-view-title" className="mt-1 break-words text-xl font-black sm:text-2xl">{booking.customer.name}</h2><div className="mt-2"><BookingStatusBadge status={booking.status} /></div></div><button data-dialog-initial-focus onClick={onClose} aria-label="Close booking details" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border"><X className="h-5 w-5" /></button></header>
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain p-4 sm:p-5">
        <section className="grid gap-3 rounded-2xl border bg-slate-50 p-4 text-sm">
          <p className="flex items-start gap-2"><CalendarClock className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /><span className="min-w-0 break-words"><strong>{formatDateTime(booking.startTime)}</strong> – {formatDateTime(booking.endTime)} <span className="text-slate-500">({booking.timezone})</span></span></p>
          <p className="flex items-start gap-2"><UserRound className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /><span className="min-w-0 break-words">{booking.service.name} with {booking.staff.name}</span></p>
          <p className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /><span className="min-w-0 break-words">{booking.location.name || (booking.bookingChannel === 'mobile' ? 'Mobile appointment' : 'Primary location')}</span></p>
          <p className="flex items-start gap-2"><CreditCard className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /><span className="min-w-0 break-words">{booking.paymentStatus} · {new Intl.NumberFormat('en-GB', { style: 'currency', currency: 'GBP' }).format(booking.quotedAmount / 100)}</span></p>
          <p className="flex items-start gap-2"><FileText className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /><span className="min-w-0 break-words">Intake: {booking.intakeStatus.replaceAll('_', ' ').toLowerCase()}</span></p>
        </section>
        {booking.status !== 'BLOCKED' && <section><h3 className="text-sm font-black uppercase tracking-wide text-slate-500">Customer</h3><div className="mt-2 rounded-xl border p-4 text-sm"><p className="break-all">{booking.customer.email || 'No email'}</p><p className="break-words">{booking.customer.phone || 'No phone'}</p></div></section>}
        ...
        {rescheduling && <form onSubmit={saveReschedule} className="grid gap-3 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 sm:grid-cols-2"><h3 className="font-black text-indigo-950 sm:col-span-2">Reschedule with keyboard-accessible controls</h3><label className="text-sm font-semibold">Date<input required type="date" value={date} onChange={event => setDate(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border p-2" /></label><label className="text-sm font-semibold">Time<input required type="time" value={time} onChange={event => setTime(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border p-2" /></label><label className="text-sm font-semibold sm:col-span-2">Team member<select value={staffId} onChange={event => setStaffId(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border bg-white p-2">{staff.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><div className="grid grid-cols-1 gap-2 sm:col-span-2 sm:grid-cols-2"><button disabled={saving} className="min-h-11 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-bold text-white">Confirm new time</button><button type="button" onClick={() => setRescheduling(false)} className="min-h-11 rounded-lg border bg-white px-4 py-2 text-sm font-bold">Keep current time</button></div></form>}
      ...
      <footer className="grid shrink-0 grid-cols-2 gap-2 border-t bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        {booking.status !== 'BLOCKED' && <button onClick={() => setRescheduling(true)} disabled={saving || ['COMPLETED','CANCELLED','NO_SHOW'].includes(booking.status)} className="min-h-11 rounded-lg border px-3 py-2 text-sm font-bold disabled:opacity-40">Reschedule</button>}
        {booking.status === 'BLOCKED' && <button onClick={() => void removeBlock()} disabled={saving} className="min-h-11 rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-700">Remove block</button>}
        {nextActions[booking.status]?.map(action => <button key={action.status} onClick={() => void updateStatus(action.status)} disabled={saving} className={`min-h-11 rounded-lg px-3 py-2 text-sm font-bold ${action.status === 'CANCELLED' ? 'border border-rose-200 text-rose-700' : 'bg-slate-900 text-white'}`}>{action.label}</button>)}
        {booking.status === 'AWAITING_PAYMENT' && <button onClick={() => onCheckout(booking)} className="min-h-11 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-bold text-white">Open checkout</button>}
      </footer>
    </aside>
  </div>;
}
