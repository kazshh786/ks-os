...
export function AgencyBookingSystemPage() {
  ...
  if (error || !workspace) {
    return <div role="alert" className="rounded-3xl border border-rose-900 bg-rose-950/30 p-6 text-rose-200"><h1 className="text-xl font-black">Agency booking workspace unavailable</h1><p className="mt-2 text-sm">{error}</p><button type="button" onClick={() => void activate()} className="mt-5 min-h-11 rounded-xl bg-white px-4 py-2 text-sm font-black text-slate-950">Try again</button></div>;
  }
