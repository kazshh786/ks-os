...
const imageCategories = new Set(['LOGO', 'TEAM_PHOTO', 'LOCATION_PHOTO', 'SERVICE_PHOTO', 'RESULT_PHOTO']);
...
type AssetPayload = {
  ...
  canUpload: boolean;
  sourceQuestionnaireReference: string | null;
  assets: Asset[];
};
...
function AssetCard({ asset, tenantReference, canManage, onChanged }: { asset: Asset; tenantReference: string; canManage: boolean; onChanged: () => Promise<void> }) {
  ...
  return <article className={`${surface} overflow-hidden`}>
    <div className="aspect-[16/10] border-b border-slate-800 bg-slate-950">
      {isImage && asset.signedViewUrl ? <img src={asset.signedViewUrl} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-slate-600">{asset.mimeType === 'application/pdf' ? <FileText className="h-12 w-12" /> : <FileBadge className="h-12 w-12" />}</div>}
    </div>
    <div className="space-y-4 p-4">
      <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-violet-700/50 bg-violet-950/30 px-2 py-1 text-[10px] font-black uppercase tracking-wide text-violet-200">{labelCategory(asset.category)}</span><span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-wide ${asset.reviewStatus === 'APPROVED' ? 'border-emerald-700 bg-emerald-950/30 text-emerald-200' : asset.reviewStatus === 'REJECTED' ? 'border-rose-700 bg-rose-950/30 text-rose-200' : 'border-amber-700 bg-amber-950/30 text-amber-200'}`}>{asset.reviewStatus === 'PENDING' ? 'Needs review' : asset.reviewStatus.toLowerCase()}</span></div><h3 className="mt-3 truncate font-black text-white" title={asset.fileName}>{asset.fileName}</h3><p className="mt-1 text-xs text-slate-500">{bytes(asset.byteSize)} · {asset.provenance.replaceAll('_', ' ').toLowerCase()}</p></div>
      ...
      {canManage ? <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Permissions</p><div className="mt-3 space-y-2 text-xs font-bold text-slate-300"><label className="flex items-center gap-2"><input type="checkbox" checked={copyright} onChange={event => setCopyright(event.target.checked)} />Copyright / usage rights confirmed</label><label className="flex items-center gap-2"><input type="checkbox" checked={publicUse} onChange={event => setPublicUse(event.target.checked)} />May appear on the public website</label><label className="flex items-center gap-2"><input type="checkbox" checked={aiUse} onChange={event => setAiUse(event.target.checked)} />May be used as AI design/generation input</label></div>{changed ? <button type="button" disabled={!copyright || Boolean(busy)} className={`${secondary} mt-3`} onClick={() => void command('permissions', () => agencyFetch(`/tenants/${tenantReference}/assets/${asset.reference}/permissions`, { method: 'PATCH', body: JSON.stringify({ publicUsePermission: publicUse, aiUsePermission: aiUse, copyrightConfirmed: copyright, consentStatus: publicUse ? 'CONFIRMED' : 'NOT_APPLICABLE' }) }))}>{busy === 'permissions' ? 'Saving…' : 'Save permissions'}</button> : null}<p className="mt-2 text-[11px] leading-5 text-slate-600">Changing permissions returns the asset to agency review.</p></div> : null}
...
export default function AgencyClientAssetLibraryPage() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const initiated = await agencyFetch(`/tenants/${data.tenant.reference}/assets`, {
        method: 'POST',
        body: JSON.stringify({ fileName: file.name, mimeType: file.type, byteSize: file.size, digestSha256, category, publicUsePermission: publicUse, aiUsePermission: aiUse, copyrightConfirmed: true, consentStatus: publicUse ? 'CONFIRMED' : 'NOT_APPLICABLE' }),
      });
  ...
  return <div className="space-y-6">
    ...
    {data && data.canUpload && canManage ? <section className={`${surface} p-5 sm:p-6`}><div className="flex items-start gap-3"><UploadCloud className="mt-1 h-6 w-6 text-violet-300" /><div><h2 className="text-xl font-black text-white">Add an asset</h2><p className="mt-1 text-sm text-slate-500">Files are verified and stored privately until agency review.</p></div></div><div className="mt-5 grid gap-4 lg:grid-cols-2"><label className="rounded-2xl border-2 border-dashed border-slate-700 bg-slate-950/60 p-6 text-center text-sm text-slate-400 transition hover:border-violet-600"><FileImage className="mx-auto h-8 w-8 text-violet-300" /><span className="mt-3 block font-black text-white">{file ? file.name : 'Choose or drop a file'}</span><span className="mt-1 block text-xs">Images, PDF or text · max 20 MB</span><input type="file" className="sr-only" accept="image/jpeg,image/png,image/webp,image/avif,application/pdf,text/plain" onChange={event => setFile(event.target.files?.[0] ?? null)} /></label><div className="space-y-3"><label className="block text-xs font-bold text-slate-400">Asset type<select value={category} onChange={event => setCategory(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white">{categories.map(group => <optgroup key={group.group} label={group.group}>{group.values.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</optgroup>)}</select></label><label className="flex items-start gap-2 text-xs font-bold text-slate-300"><input className="mt-0.5" type="checkbox" checked={copyright} onChange={event => setCopyright(event.target.checked)} /><span>I confirm the client/agency has the rights needed to store and use this file.</span></label><label className="flex items-center gap-2 text-xs font-bold text-slate-300"><input type="checkbox" checked={publicUse} onChange={event => setPublicUse(event.target.checked)} />Allow public website use</label><label className="flex items-center gap-2 text-xs font-bold text-slate-300"><input type="checkbox" checked={aiUse} onChange={event => setAiUse(event.target.checked)} />Allow as AI design/generation input</label><button type="button" className={primary} disabled={!file || !copyright || busy} onClick={() => void upload()}>{busy ? <><Loader2 className="h-4 w-4 animate-spin" />Uploading…</> : <><UploadCloud className="h-4 w-4" />Upload privately</>}</button></div></div></section> : null}
    {data ? <div className="space-y-8">{grouped.map(group => <section key={group.group}><div className="mb-3 flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-xl border border-slate-800 bg-slate-900 text-violet-300">{group.group === 'Brand' ? <ShieldCheck className="h-4 w-4" /> : group.group === 'Photography' ? <ImageIcon className="h-4 w-4" /> : <FileText className="h-4 w-4" />}</div><div><h2 className="font-black text-white">{group.group}</h2><p className="text-xs text-slate-500">{group.assets.length} asset{group.assets.length === 1 ? '' : 's'}</p></div></div>{group.assets.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{group.assets.map(asset => <AssetCard key={asset.reference} asset={asset} tenantReference={data.tenant.reference} canManage={canManage} onChanged={load} />)}</div> : <div className="rounded-2xl border border-dashed border-slate-800 p-6 text-sm text-slate-600">No {group.group.toLowerCase()} assets yet.</div>}</section>)}</div> : null}
  </div>;
}
