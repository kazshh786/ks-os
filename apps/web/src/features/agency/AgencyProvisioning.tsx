...
const DRAFT_REFERENCE_STORAGE_KEY = 'ks-os-provisioning-draft';
const FORM_STORAGE_KEY = 'ks-os-provisioning-form';
...
function loadLocalForm() {
  try {
    ...
    return {
      ...DEFAULT_FORM,
      ...stored,
      requestedPageTypes: Array.isArray(stored.requestedPageTypes) ? stored.requestedPageTypes : DEFAULT_FORM.requestedPageTypes,
    };
  } catch {
    return DEFAULT_FORM;
  }
...
export function AgencyProvisioningPage() {
  ...
  const [draftReference, setDraftReference] = useState(() => localStorage.getItem(DRAFT_REFERENCE_STORAGE_KEY) || '');
  const [run, setRun] = useState<any>(null);
  const [validation, setValidation] = useState<any>(null);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  const [form, setForm] = useState(loadLocalForm);
  ...
  const save = async () => {
    ...
    if (result?.reference) { setDraftReference(result.reference); localStorage.setItem(DRAFT_REFERENCE_STORAGE_KEY, result.reference); }
    return result;
  };
  const resume = async () => {
  ...
  const validate = async () => {
    ...
    const reference = draftReference || localStorage.getItem(DRAFT_REFERENCE_STORAGE_KEY);
    if (!reference) return;
  ...
  const start = async () => {
    ...
    const idempotencyStorageKey = `ks-os-provisioning-idempotency:${draftReference}`;
    const idempotencyKey = localStorage.getItem(idempotencyStorageKey) || `agency-ui:${draftReference}:${crypto.randomUUID()}`;
    localStorage.setItem(idempotencyStorageKey, idempotencyKey);
    const result = await command(() => agencyFetch('/provisioning-runs', {
      method: 'POST',
      body: JSON.stringify({ provisioningDraftReference: draftReference, idempotencyKey }),
    }));
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-violet-300">Website delivery</p><h1 className="mt-1 text-2xl font-black">Provision website and workspace</h1><p className="mt-2 max-w-3xl text-sm text-slate-400">Use this after the client business exists and fact-finding is complete. It creates the booking and website draft records from an approved production brief; it does not publish the website.</p><p className="mt-2 text-xs text-slate-500">Your entries are saved locally as you type and restored after a genuine browser reload.</p></div><div className="flex flex-wrap gap-2"><Link to="/agency/tenants/new" className="rounded-xl border border-violet-700 px-4 py-2 text-xs font-black text-violet-200">Start client onboarding</Link><Link to="/agency/fact-finding" className="rounded-xl border border-slate-700 px-4 py-2 text-xs font-black">Open fact-finding</Link></div></div>
    </div>
    ...
    <ol aria-label="Provisioning steps" className="grid gap-2 md:grid-cols-3 xl:grid-cols-5">{WIZARD_STEPS.map((label, index) => {
      ...
      return <li key={label}><button type="button" onClick={() => setCurrent(index)} className={`h-full w-full rounded-xl border p-3 text-left text-xs ${tone[state]}`}><span className="block text-[10px] font-black uppercase">{index + 1} · {state.replaceAll('_', ' ')}</span><strong className="mt-1 block">{label}</strong></button></li>;
    })}</ol>
