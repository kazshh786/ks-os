...
export function AgencyFactFindingPage() {
  ...
  const [reviewAction, setReviewAction] = useState<{ reference: string; type: 'CLARIFY' | 'REJECT' } | null>(null);
  ...
  const approve = async (response: any) => {
    await run(() => agencyFetch(`/fact-finding/responses/${response.reference}/approve`, {
      method: 'POST',
      body: JSON.stringify({ publicUseEligible: true, bookingUseEligible: true, generationUseEligible: true, note: 'Approved during controlled agency review.' }),
    }), () => void loadReview());
  };
  ...
  const submitReviewAction = async () => {
    ...
    const path = reviewAction.type === 'CLARIFY' ? 'request-clarification' : 'reject';
    const payload = reviewAction.type === 'CLARIFY' ? { message: reviewNote.trim(), requiredResponseType: 'LONG_TEXT', evidenceRequested: false } : { reason: reviewNote.trim() };
  ...
  const reset = () => {
    setQuestionnaire(null); setManualForm(null); setAnswers({}); setResponses([]); setBrief(null); setNotice(''); setError(''); setParticipantName(''); setParticipantEmail('');
  };
  ...
  return <div className="space-y-5">
    ...
    {questionnaire && !manualForm && <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-violet-300">Current intake</p><h2 className="mt-2 text-2xl font-black">{questionnaire.tenantName}</h2><p className="mt-1 text-xs text-slate-500">Version {questionnaire.version} · {questionnaire.participants?.[0]?.displayName || participantName || 'Agency-assisted'}</p></div>{statusBadge(questionnaire.status)}</div><div className="mt-5 flex flex-wrap gap-2"><button onClick={() => void loadReview()} className="rounded-xl border border-slate-700 px-4 py-2 text-xs font-black">Refresh progress</button>{['PREQUALIFIED', 'IN_PROGRESS', 'CLARIFICATION_REQUIRED'].includes(questionnaire.status) && mode === 'ASSISTED' && <button onClick={() => void run(() => loadManualForm(questionnaire.reference))} className="rounded-xl bg-violet-600 px-4 py-2 text-xs font-black">Continue assisted intake</button>}<button onClick={reset} className="rounded-xl border border-slate-700 px-4 py-2 text-xs font-black text-slate-400">Start another intake</button></div></section>}

    {questionnaire?.uploads?.length > 0 && <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6"><div><p className="text-xs font-black uppercase tracking-widest text-slate-500">Asset review</p><h2 className="mt-2 text-xl font-black">Review private files</h2><p className="mt-2 text-sm text-slate-400">Approve only safe files that may enter the production brief under their recorded permissions.</p></div><div className="mt-5 grid gap-3 md:grid-cols-2">{questionnaire.uploads.map((upload: any) => <article key={upload.reference} className="rounded-2xl border border-slate-800 bg-slate-950 p-4"><div className="flex items-start justify-between gap-3"><div><strong className="break-all text-sm">{upload.fileName}</strong><p className="mt-1 text-xs text-slate-500">{upload.category.replaceAll('_', ' ')} · {Math.max(1, Math.round(Number(upload.byteSize || 0) / 1024))} KB</p></div>{statusBadge(upload.reviewStatus)}</div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-lg border border-slate-800 px-2 py-1 text-[10px] font-black text-slate-400">{upload.uploadStatus}</span><span className="rounded-lg border border-slate-800 px-2 py-1 text-[10px] font-black text-slate-400">SCAN {upload.scanStatus}</span></div>{upload.reviewStatus === 'PENDING' && upload.uploadStatus === 'UPLOADED' && <div className="mt-4 flex gap-2"><button disabled={busy} onClick={() => void reviewUpload(upload.reference, 'APPROVED')} className="rounded-xl bg-emerald-700 px-3 py-2 text-xs font-black disabled:opacity-40">Approve asset</button><button disabled={busy} onClick={() => void reviewUpload(upload.reference, 'REJECTED')} className="rounded-xl border border-rose-800 px-3 py-2 text-xs font-black text-rose-300 disabled:opacity-40">Reject</button></div>}</article>)}</div></section>}

    {responses.length > 0 && <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-slate-500">Controlled review</p><h2 className="mt-2 text-xl font-black">Verify each usable fact</h2><p className="mt-2 text-sm text-slate-400">Nothing reaches booking or website generation until it is approved here.</p></div><button onClick={() => void buildBrief()} className="rounded-xl bg-violet-600 px-4 py-3 text-xs font-black">Generate production brief</button></div><div className="mt-5 space-y-3">{responses.map(response => { const question = questionnaire.questions?.find((item: any) => item.reference === response.questionReference); return <article key={response.reference} className="rounded-2xl border border-slate-800 bg-slate-950 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><strong>{question?.label || response.fieldMapping || 'Client fact'}</strong><p className="mt-1 text-xs text-slate-500">{response.source === 'CLIENT_PROVIDED' ? 'Client provided' : 'Agency assisted'} · {response.fieldMapping || 'Private intake'}</p></div>{statusBadge(response.status)}</div><pre className="mt-4 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-900 p-4 text-xs text-slate-300">{JSON.stringify(response.answer, null, 2)}</pre>{response.status !== 'AGENCY_APPROVED' && <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => void approve(response)} className="rounded-xl bg-emerald-700 px-3 py-2 text-xs font-black">Approve fact</button><button onClick={() => { setReviewAction({ reference: response.reference, type: 'CLARIFY' }); setReviewNote(''); }} className="rounded-xl border border-amber-700 px-3 py-2 text-xs font-black text-amber-300">Ask for clarification</button><button onClick={() => { setReviewAction({ reference: response.reference, type: 'REJECT' }); setReviewNote(''); }} className="rounded-xl border border-rose-800 px-3 py-2 text-xs font-black text-rose-300">Reject</button></div>}</article>; })}</div></section>}

    {reviewAction && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/80 p-4"><section role="dialog" aria-modal="true" className="w-full max-w-lg rounded-3xl border border-slate-700 bg-slate-900 p-6"><h2 className="text-xl font-black">{reviewAction.type === 'CLARIFY' ? 'Request clarification' : 'Reject this fact'}</h2><p className="mt-2 text-sm text-slate-400">Write a clear reason that will make sense later in the review history.</p><textarea autoFocus value={reviewNote} onChange={event => setReviewNote(event.target.value)} rows={5} className="mt-4 w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm"/><div className="mt-4 flex justify-end gap-2"><button onClick={() => setReviewAction(null)} className="rounded-xl border border-slate-700 px-4 py-2 text-xs font-black">Cancel</button><button disabled={!reviewNote.trim() || busy} onClick={() => void submitReviewAction()} className={`rounded-xl px-4 py-2 text-xs font-black ${reviewAction.type === 'CLARIFY' ? 'bg-amber-600 text-slate-950' : 'bg-rose-700'}`}>Confirm</button></div></section></div>}
