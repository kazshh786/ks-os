...
export function AgencyLaunchJourneyV3({ tenantReference, tenantDetail, onBack }: { tenantReference: string; tenantDetail: any; onBack: () => void }) {
  ...
  else if (!assetsComplete) { title = 'Review brand and images'; description = 'Upload and approve the client logo, team or location imagery and supporting assets before design work.'; action = <Link className={primary} to={discoveryHref}>Review assets <ArrowRight className="h-4 w-4" /></Link>; }
  ...
  else if (!searchComplete) { title = researchRequired ? 'Add real search research' : 'Review the search strategy'; description = researchRequired ? 'The planning draft needs governed SERP and keyword evidence before it can be approved.' : 'Review the page-by-page search plan and explicitly approve it before website generation.'; action = <Link className={primary} to={`${websiteHref}?view=search`}>Open search strategy <ArrowRight className="h-4 w-4" /></Link>; }
...
function SearchStrategyWorkspace({ siteReference, siteName, blueprint, pageTitles }: { siteReference: string; siteName: string; blueprint: any; pageTitles: Record<string, string> }) {
  ...
  const importResearch = async (file: File) => { setBusy('import'); setError(''); try { const bundle = JSON.parse(await file.text()); await agencyFetch(`/sites/${siteReference}/search-intelligence/strategies`, { method: 'POST', body: JSON.stringify(bundle) }); setNotice('Research imported as a governed draft. Review it before approval.'); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'The research file could not be imported.'); } finally { setBusy(''); } };
  ...
  if (missing) { const approved = blueprint?.status === 'APPROVED'; return <Guidance tone={approved ? 'info' : 'warning'} title={approved ? 'Create the search planning draft' : 'Approve the website structure first'}><p>{approved ? 'KS OS will create one planning brief for every approved page. Real SERP and keyword research is still required before approval.' : `Search planning is tied to the exact pages you approve. ${blueprint ? `Website structure revision ${blueprint.revision} is currently ${String(blueprint.status).replaceAll('_', ' ').toLowerCase()}.` : 'No website structure is available yet.'}`}</p>{approved ? <button type="button" className={`${primary} mt-4`} disabled={!canManage || Boolean(busy)} onClick={() => void createDraft()}>{busy === 'create' ? 'Creating…' : 'Create search planning draft'}</button> : null}</Guidance>; }
  const researchRequired = data.researchReadiness?.status === 'RESEARCH_REQUIRED' || Number(data.researchFreshness?.evidenceCount || 0) === 0;
  return <div className="space-y-5">{notice ? <Guidance tone="success" title="Saved"><p>{notice}</p></Guidance> : null}{researchRequired ? <Guidance tone="warning" title="Add real search research before approval"><p>This planning draft only knows the approved website structure. Import a governed research bundle containing the strategy, one brief per page and referenced evidence.</p>{canManage ? <label className={`${primary} mt-4 cursor-pointer`}><FileSearch className="h-4 w-4" />{busy === 'import' ? 'Importing…' : 'Import research JSON'}<input className="sr-only" type="file" accept="application/json,.json" disabled={Boolean(busy)} onChange={event => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file) void importResearch(file); }} /></label> : null}</Guidance> : <Guidance tone="success" title="Research evidence is attached"><p>{data.researchFreshness.evidenceCount} evidence record{data.researchFreshness.evidenceCount === 1 ? '' : 's'} are bound to this strategy. Review the page briefs before approval.</p></Guidance>}<section className={`${surface} p-5`}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Search strategy</p><h2 className="mt-1 text-xl font-black text-white">{siteName}</h2></div><StatusPill value={data.status === 'APPROVED' ? 'COMPLETE' : researchRequired ? 'NEEDS_YOU' : 'READY'} /></div><p className="mt-4 text-sm text-slate-400">{data.strategy.targetAudience?.segments?.map((segment: any) => segment.name).join(' · ') || 'Audience not defined'} · {data.strategy.searchMarket?.locale || 'Market not defined'} · {data.researchFreshness?.evidenceCount || 0} evidence records</p></section><section className={`${surface} overflow-hidden`}><div className="border-b border-slate-800 px-5 py-4"><h3 className="font-black text-white">Page search briefs</h3><p className="mt-1 text-xs text-slate-500">Every approved website page owns a clear search purpose.</p></div><div className="divide-y divide-slate-800">{(data.briefs || []).map((brief: any) => <div key={brief.reference} className="flex flex-col gap-2 px-5 py-4 md:flex-row md:items-center md:justify-between"><div><p className="font-black text-white">{pageTitles[brief.pageReference] || brief.primaryTopic || brief.pageType}</p><p className="mt-1 text-xs text-slate-500">Primary target: {brief.primaryKeyword} · {String(brief.primarySearchIntent || '').replaceAll('_', ' ').toLowerCase()}</p></div><span className="text-xs font-bold text-slate-400">{String(brief.status).replaceAll('_', ' ')}</span></div>)}</div></section>{data.status !== 'APPROVED' && !researchRequired && canManage ? <button type="button" className={primary} disabled={Boolean(busy)} onClick={() => void approve()}>{busy === 'approve' ? 'Approving…' : 'Approve search strategy'}</button> : null}</div>;
}
