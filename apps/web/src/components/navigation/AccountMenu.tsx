...
export const AccountMenu: React.FC<AccountMenuProps> = ({ displayName, email, roleLabel, settingsHref, tone = 'light', compact = false, onSignOut }) => {
  ...
  return <details className="group relative">
    <summary aria-label="Open account menu" className={`flex cursor-pointer list-none items-center rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${compact ? 'justify-center p-1' : 'gap-3 p-2'} ${dark ? 'hover:bg-slate-900' : 'hover:bg-slate-100'}`}>
    ...
    <div className={`absolute bottom-[calc(100%+0.5rem)] left-0 z-50 w-64 rounded-2xl border p-2 shadow-2xl ${dark ? 'border-slate-700 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-950'}`}>
      ...
      <Link to={settingsHref} className={`mt-1 flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold ${dark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'}`}><Settings2 aria-hidden="true" className="h-4 w-4" />Account settings</Link>
      <button type="button" onClick={onSignOut} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-bold ${dark ? 'text-rose-300 hover:bg-slate-800' : 'text-rose-700 hover:bg-slate-100'}`}><LogOut aria-hidden="true" className="h-4 w-4" />Sign out</button>
    </div>
  </details>;
};
