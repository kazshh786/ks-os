...
export async function GET(request:Request,{params}:{params:Promise<{tenantId:string;reference:string}>}){
  ...
  try{
    const {data,error}=await serviceClient().from('appointments').select('public_reference,status,payment_status,start_time,end_time').eq('tenant_id',tenantId).eq('public_reference',reference).single();
    if(error||!data)return publicError(404,'BOOKING_NOT_FOUND','Booking not found');
    return Response.json({booking:{reference:data.public_reference,status:data.status,paymentStatus:data.payment_status,startTime:data.start_time,endTime:data.end_time}},{headers:{'Cache-Control':'no-store'}});
