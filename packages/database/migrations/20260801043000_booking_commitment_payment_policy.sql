-- Public online bookings use a commitment-first payment policy.
-- Paid services require either a configurable deposit or full payment through
-- Stripe. Free services are confirmed without creating a payment obligation.

ALTER TABLE booking_pages
  ALTER COLUMN payment_settings
  SET DEFAULT '{"mode":"DEPOSIT","depositPercentage":20,"promotionCodesEnabled":false,"giftCardsEnabled":false}'::jsonb;
...
SET payment_settings = jsonb_set(
  ...
  to_jsonb(CASE
    WHEN COALESCE(payment_settings->>'depositPercentage', '') ~ '^[0-9]+([.][0-9]+)?$'
      AND (payment_settings->>'depositPercentage')::numeric >= 1
      AND (payment_settings->>'depositPercentage')::numeric <= 99
      THEN round((payment_settings->>'depositPercentage')::numeric)::integer
    ELSE 20
  END),
  true
)
...
BEGIN
  ...
  SELECT
    s.duration,
    greatest(0, s.price - coalesce(s.discount, 0))
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'SERVICE_NOT_AVAILABLE'
      USING ERRCODE = 'P0001', DETAIL = 'Service is not active or available.';
  END IF;
...
COMMENT ON COLUMN booking_pages.payment_settings IS
  'Public paid bookings use DEPOSIT (1-99 percent) or FULL. Free services bypass payment automatically.';
