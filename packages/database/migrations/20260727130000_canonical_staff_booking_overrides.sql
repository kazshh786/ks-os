-- Migration 38: Canonical Staff Booking Overrides for create_public_booking
-- Incorporates staff_pricing (custom_price_in_cents, custom_duration_minutes) resolution
-- Preserves all Prompt 2 concurrency and safety guarantees:
-- pg_advisory_xact_lock, idempotency, overlap check, status filtering, hold integrity, and tenant isolation
...
BEGIN
  ...
  -- 4. Re-validate Active Service & Staff Override
  ...
  -- Query staff_pricing override scoped to tenant_id via users table
  SELECT sp.custom_price_in_cents, sp.custom_duration_minutes
  INTO v_custom_price, v_custom_duration
  FROM staff_pricing sp
  ...
  IF FOUND THEN
    IF v_custom_price IS NOT NULL THEN
      v_service_price := v_custom_price;
    END IF;
    IF v_custom_duration IS NOT NULL THEN
      v_service_duration := v_custom_duration;
    END IF;
  END IF;
  ...
  -- 10. Insert Appointment with Effective Price & Duration
