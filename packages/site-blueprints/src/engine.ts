...
function buildCandidates(
  ...
  const baseline = (
    ...
    if (!ready && missingMessage) {
      actionItems.push(actionItem(
        'BUSINESS_PROFILE',
        pageType === 'HOME' ? 'BLOCKING' : 'WARNING',
    ...
    candidates.push(candidate({
      ...
      required: ['HOME', 'BOOKING'].includes(pageType),
      scoreInput: {
        pageType,
        requiredBaseline: ['HOME', 'BOOKING'].includes(pageType),
  ...
  baseline(
    ...
    'Complete the business profile before generating a strong About page.',
  );
  baseline(
    'CONTACT',
...
export function generateBlueprintPlan(input: BlueprintEngineInput): BlueprintPlan {
  ...
  const selected = [
    ...marketing.filter((item) => item.score >= 0).slice(0, input.marketingPageLimit),
    ...nonMarketing,
  ].sort((left, right) =>
    PAGE_DEFAULTS[left.pageType].priority - PAGE_DEFAULTS[right.pageType].priority
    || right.score - left.score);
