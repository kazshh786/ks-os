...
export const BusinessTypeSchema = z.enum(["SALON_BARBER","BEAUTY_AESTHETICS","LOGISTICS_COURIER","AGENCY","CONSULTANCY","PLUMBING","ELECTRICAL","CLEANING","CONSTRUCTION","PROPERTY_MANAGEMENT","ESTATE_AGENCY","RESTAURANT_CAFE","RETAIL","ECOMMERCE","GYM_PT","EDUCATION_TUTORING","MADRASSA","CHARITY_NONPROFIT","GARAGE_MECHANIC","PROFESSIONAL_SERVICES"] as const);
...
export type ModuleDefinition = {
  key: ModuleKey; defaultLabel: string; status: 'implemented' | 'foundation' | 'planned';
  route: string | null; capabilities: readonly string[]; ownerOnly: boolean;
  entitlements: readonly string[]; recommendedBusinessTypes: readonly BusinessType[];
};
...
export const ProductOnboardingConfigurationSchema = z.object({
  version: z.literal(1), completedAt: z.string().datetime(), answers: ProductOnboardingAnswersSchema,
}).strict();
export type ProductOnboardingConfiguration = z.infer<typeof ProductOnboardingConfigurationSchema>;

export const BusinessProfileSchema = z.object({
  businessType: BusinessTypeSchema.nullable(), displayName: z.string(),
  ...
  enabledModules: z.array(ModuleKeySchema), recommendedModules: z.array(ModuleKeySchema),
...
export function resolveBusinessProfile(businessType: unknown, configuration?: unknown): BusinessProfile {
  ...
  const defaults: BusinessProfile['onboardingDefaults'] = {
    teamSize:'1',buying:hasBookings?['appointments']:['quotes'],
    delivery: seed.operatingModel === 'cases' ? ['jobs'] : [seed.operatingModel],
    resources:['staff'],payment:hasBookings?['card','pos']:['invoices'],
    manage:['customers','staff','money','reports','automation'],
  };
  return {
    businessType:type,displayName:BUSINESS_TYPES.find(item=>item.key===type)?.label ?? 'Business',
