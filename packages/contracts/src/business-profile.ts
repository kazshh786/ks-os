...
type BusinessTypeDefinition = { key: BusinessType; label: string; aliases: readonly string[] };
...
export const ModuleKeySchema = z.enum([
  'dashboard','crm','sales','bookings','services','calendar','work','tasks','projects','operations','forms','documents','communications','email-marketing','social','payments','pos','finance','inventory','team','analytics','reports','reputation','automations','integrations','fleet','routes','dispatch','assets','locations','booking-page','settings','security',
]);
...
type ImplementedModule = [label: string, route: string, capabilities: string[], ownerOnly: boolean, entitlements: string[]];
const implemented: Partial<Record<ModuleKey, ImplementedModule>> = {
...
const modelModules: Record<string, ModuleKey[]> = {
  ...
  orders: ['pos','inventory','sales','documents'],
  cases: ['sales','documents'],
};

type Terminology = { customer:string; customers:string; work:string; works:string; staff:string; staffPlural:string };
type ProfileSeed = { operatingModel:string; terminology:Terminology };
const seed = (operatingModel:string, customer:string, customers:string, work:string, works:string, staff:string, staffPlural:string): ProfileSeed => ({ operatingModel, terminology:{customer,customers,work,works,staff,staffPlural} });
const profileSeeds: Record<BusinessType, ProfileSeed> = {
  SALON_BARBER: seed('appointments','Customer','Customers','Appointment','Appointments','Practitioner','Practitioners'),
  BEAUTY_AESTHETICS: seed('appointments','Client','Clients','Appointment','Appointments','Practitioner','Practitioners'),
  LOGISTICS_COURIER: seed('deliveries','Customer','Customers','Delivery','Deliveries','Driver','Drivers'),
  AGENCY: seed('projects','Client','Clients','Project','Projects','Team member','Team members'),
  CONSULTANCY: seed('projects','Client','Clients','Project','Projects','Consultant','Consultants'),
  PLUMBING: seed('jobs','Customer','Customers','Job','Jobs','Engineer','Engineers'),
  ELECTRICAL: seed('jobs','Customer','Customers','Job','Jobs','Engineer','Engineers'),
  CLEANING: seed('jobs','Customer','Customers','Job','Jobs','Team member','Team members'),
  CONSTRUCTION: seed('projects','Client','Clients','Project','Projects','Team member','Team members'),
  PROPERTY_MANAGEMENT: seed('jobs','Client','Clients','Job','Jobs','Team member','Team members'),
  ESTATE_AGENCY: seed('cases','Client','Clients','Case','Cases','Agent','Agents'),
  RESTAURANT_CAFE: seed('orders','Customer','Customers','Order','Orders','Team member','Team members'),
  RETAIL: seed('orders','Customer','Customers','Order','Orders','Team member','Team members'),
  ECOMMERCE: seed('orders','Customer','Customers','Order','Orders','Team member','Team members'),
  GYM_PT: seed('appointments','Member','Members','Session','Sessions','Trainer','Trainers'),
  EDUCATION_TUTORING: seed('classes','Student','Students','Class','Classes','Teacher','Teachers'),
  MADRASSA: seed('classes','Student','Students','Class','Classes','Teacher','Teachers'),
  CHARITY_NONPROFIT: seed('cases','Member','Members','Case','Cases','Volunteer','Volunteers'),
  GARAGE_MECHANIC: seed('jobs','Customer','Customers','Job','Jobs','Mechanic','Mechanics'),
  PROFESSIONAL_SERVICES: seed('cases','Client','Clients','Case','Cases','Team member','Team members'),
};
...
const manageModules: Record<ProductOnboardingAnswers['manage'][number], ModuleKey[]> = {
  ...
  jobs:['work'], projects:['projects'], staff:['team'], calendar:['calendar'], money:['finance','payments'],
...
export function resolveBusinessProfile(businessType: unknown, configuration?: unknown): BusinessProfile {
  ...
  const profileSeed = profileSeeds[type ?? (configured ? 'PROFESSIONAL_SERVICES' : 'SALON_BARBER')];
  const recommendation = [...new Set([...core, ...(modelModules[profileSeed.operatingModel] ?? [])])];
  ...
  const defaults: BusinessProfile['onboardingDefaults'] = {
    ...
    delivery:profileSeed.operatingModel === 'cases' ? ['jobs'] : [profileSeed.operatingModel as ProductOnboardingAnswers['delivery'][number]],
    resources:['staff'],
  ...
  return {
    ...
    terminology:{...profileSeed.terminology},
    ...
    recommendedOperatingModel:configured?.delivery[0] ?? profileSeed.operatingModel,
    optionalEngines:enabled.filter(key => !core.includes(key)),
    pipelineMetadata:{status:enabled.includes('sales') ? 'implemented' : 'planned', workLabel:profileSeed.terminology.work},
