...
function sectionClass(section: SiteSection, component: string) {
  return `site-section ${component}${section.variant ? ` variant-${section.variant}` : ''}`;
}
...
export function MobileNavigation(context: ComponentRenderContext): SafeHtml {
  const items = context.snapshot.navigation.primary
    .map((item) => html`<li><a href="${escapeHtml(pageHref(context, item.pageReference))}">${escapeHtml(item.label)}</a></li>`)
    .join('');
...
export function Introduction(section: SectionOf<'INTRODUCTION'>): SafeHtml {
  return html`<section class="${sectionClass(section, 'introduction')}"><h2>${escapeHtml(section.heading)}</h2><p>${escapeHtml(section.body)}</p></section>`;
}
...
export function Benefits(section: SectionOf<'BENEFITS'>): SafeHtml {
  const items = section.items
    .map((item) => html`<li><h3>${escapeHtml(item.heading)}</h3><p>${escapeHtml(item.body)}</p></li>`)
    .join('');
  return html`<section class="${sectionClass(section, 'benefits')}"><h2>${escapeHtml(section.heading)}</h2><ul class="feature-grid">${items}</ul></section>`;
}

export function Process(section: SectionOf<'PROCESS'>): SafeHtml {
  const steps = section.steps
    ...
    .join('');
  return html`<section class="${sectionClass(section, 'process')}"><h2>${escapeHtml(section.heading)}</h2><ol>${steps}</ol></section>`;
}
...
export function LocationDetails(
  ...
  const address = [...location.addressLines, location.locality, location.region, location.postalCode]
    ...
    .join('<br>');
  return html`<section class="${sectionClass(section, 'location-details')}"><h2>${escapeHtml(section.heading)}</h2><h3>${escapeHtml(location.name)}</h3><address>${address}</address></section>`;
}
...
export function OpeningHours(
  ...
  return html`<section class="${sectionClass(section, 'opening-hours')}"><h2>${escapeHtml(section.heading)}</h2><table><tbody>${rows}</tbody></table></section>`;
}
...
export function ContactDetails(
  ...
  const actions = section.secondaryActions
    ...
    .join('');
  return html`<section class="${sectionClass(section, 'contact-details')}"><h2>${escapeHtml(section.heading)}</h2>${section.body ? `<p>${escapeHtml(section.body)}</p>` : ''}<div class="action-row">${actions}</div></section>`;
}
...
export function BookingCallToAction(
  ...
  return html`<section class="${sectionClass(section, 'booking-call-to-action')}"><h2>${escapeHtml(section.heading)}</h2><p>${escapeHtml(section.body)}</p>${renderAction(section.primaryAction, context, 'button primary final-booking')}</section>`;
}
...
export function SiteFooter(
  ...
  const links = context.snapshot.navigation.footer
    .map((item) => html`<li><a href="${escapeHtml(pageHref(context, item.pageReference))}">${escapeHtml(item.label)}</a></li>`)
    .join('');
  return html`<footer class="${sectionClass(section, 'site-footer')}"><div><strong>${escapeHtml(context.snapshot.business.name)}</strong>${section.legalText ? `<p>${escapeHtml(section.legalText)}</p>` : ''}</div><nav aria-label="Footer navigation"><ul>${links}</ul></nav>${renderAction(section.primaryAction, context, 'button primary footer-booking')}</footer>`;
}
...
export function renderSection(
  ...
  switch (section.type) {
    ...
    case 'INTRODUCTION': return Introduction(section);
    ...
    case 'PROCESS': return Process(section);
