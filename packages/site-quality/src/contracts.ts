...
export const SiteQualityPerformanceMetricSchema = z.object({
  ...
  measurementMode: z.enum(['LAB', 'FIELD']).default('LAB'),
...
export const BrowserAuditPageResultSchema = z.object({
  ...
  lcpElementTag: z.string().trim().min(1).max(80).nullable().default(null),
  lcpResourceUrl: z.string().max(2_000).nullable().default(null),
  lcpResourceTransferBytes: z.number().int().nonnegative().nullable().default(null),
  lcpImageLoading: z.enum(['eager', 'lazy', 'auto']).nullable().default(null),
  lcpImageHasResponsiveSource: z.boolean().nullable().default(null),
  lcpImageHasIntrinsicDimensions: z.boolean().nullable().default(null),
  lcpResourceDiscoverable: z.boolean().default(true),
  lcpResourceFailed: z.boolean().default(false),
  longMainThreadTaskCount: z.number().int().nonnegative().default(0),
  longMainThreadTaskTotalMs: z.number().finite().nonnegative().default(0),
  renderBlockingStylesheetCount: z.number().int().nonnegative().default(0),
  parserBlockingScriptCount: z.number().int().nonnegative().default(0),
  slowFontResourceCount: z.number().int().nonnegative().default(0),
